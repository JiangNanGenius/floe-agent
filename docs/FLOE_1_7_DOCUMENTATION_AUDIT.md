...
检查：六份改动文档的 203 个相对链接全部存在；`git diff --check` 无空白错误；`rg` 复核无遗留“225/226 为当前交付”的现在时表述，本轮新增文字不含私有目录路径、凭据或未脱敏诊断；所引运行号、提交与产物摘要均取自仓库内已提交的交付记录、`FloeAgent/ThirdParty/Collabora/engine.lock.json` 与 `feather.json`；`test_office_host_pin_path.py`（11 项通过）与 `test_feather_source.py`（4 项通过）复核了新宿主固定与 Feather 源的路径/来源契约。

范围外/未宣称：不修改 `README.md`、`README.zh-CN.md`、`docs/README.md`、`docs/USER_GUIDE*`、`docs/RELEASE_NOTES_*`；不创建或移动标签、不推送、不上传；不把 Build 227 之后的源码修复写成已构建、已上传或已真机通过；不宣称 Build 228 已有 App 构建、IPA、签名上传、Apple 处理或 TestFlight 记录（`project.yml` 中预留的下一构建号不是交付）。

## 2026-09-23 Build 225：修复 Build 224 验收 SDK 类型化抛错编译失败 / Build 224 accepted-SDK typed-throws compile repair

Build 225 moves all four shipping targets to 1.7.0 (225); `FloeAgent/project.yml` carries four `CURRENT_PROJECT_VERSION: "225"` entries and the regenerated `FloeAgent.xcodeproj` matches (eight Debug/Release entries; regeneration idempotent), with `Package.resolved` untouched. The only code change is in `FloeAgent/FloeApp/Execution/LinuxGuestImageDownloader.swift`: the `do/catch` around the untyped-throws `URLSession.bytes(for:)` seam is now exhaustive inside the `throws(LinuxGuestImageTransferError)` function (non-`URLError` failures explicitly classify as `.responseInvalid`, which stays fail-closed), and `URLError.cancelled` maps to `.cancelled` only while `Task.isCancelled` is true, so a cancellation can never masquerade as a mirror-eligible availability failure; the streaming catch order and the `.networkFailure`/`.serverUnavailable`-only switching rule are unchanged. Build 224's accepted-SDK App build (Xcode 26.6, iPhoneOS 26.5) failed on exactly this line (`LinuxGuestImageDownloader.swift:101:9: error: thrown expression type 'any Error' cannot be converted to error type 'LinuxGuestImageTransferError'`); its immutable tag `v1.7.0-beta.81` (`c36b7b24`) and failed run 35767875337 (exit 65, no IPA or upload, publish jobs skipped) are retained as the failure record and documented in the new bilingual `RELEASE_NOTES_1.7.0_BUILD_225.md` (with a failure table), `TESTFLIGHT_1.7_WHATS_NEW_BUILD_225.json` (validated) and the updated `TESTFLIGHT_1.7.0_BETA.md`. Evidence recorded: 23 focused Linux image tests via the already-built FloeExecutionTests bundle under the Xcode 27.0/Swift 6.2 toolchain (11 mirror contract, 6 shard staging, 4 download coalescing, 2 install state — 0 failures), plus a Swift 6 iOS 26.0 Simulator-SDK object compile of the real repaired App source against a public-ABI FloeExecution contract stub that also reproduces the original error when compiling the Build 224 tagged file. Cloud App build/package, upload, Apple processing, TestFlight availability, release publication and physical-device behavior are explicitly not claimed; no tag (`v1.7.0-beta.82` is only the planned name), workflow dispatch or release happened, and Build 224 metadata is retained unchanged. `articles/`, `output/`, private credentials, the website checkout and unrelated worktrees were untouched.
