...
上述为候选源码行为，构建号已准备为 170，完整 App 回归与新 TestFlight 可用状态尚未确认。APT 官方签名仓库仍未发布，不能把 PyPI/npm 可用性等同于完整 Linux 软件包生态。当前证据和限制见[反馈修复记录](FLOE_156_FEEDBACK_REPAIR.md)。
