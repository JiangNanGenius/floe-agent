...
build 172 已上传成功。固定源码通过 1,270 次 Swift 测试执行、159 项 SDK 27 完整 App 回归及该 SDK 的 iPad/iPhone 手记 UI 用例；加急上传按用户要求跳过上传 SDK 的模拟器验收，Apple 处理和新 TestFlight 可安装状态仍待确认。APT 官方签名仓库仍未发布，不能把 PyPI/npm 可用性等同于完整 Linux 软件包生态。当前证据和限制见[反馈修复记录](FLOE_156_FEEDBACK_REPAIR.md)。
