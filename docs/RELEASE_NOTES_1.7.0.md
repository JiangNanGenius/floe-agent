...
状态：1.7.0（172）已交付 Floe QA 内部 TestFlight，VALID / IN_BETA_TESTING 已确认。应用源码为 `fb86fef` / `v1.7.0-beta.29`，本次直传打包策略为 `9c741b0`；上传使用 Xcode 26.6 / SDK 26.5，部分 27 专属接口走兼容入口。正式发布另行安排；完整包/模型交付和真机验收仍有未完成内容。

## Build 172 反馈修复（已交付内部 TestFlight）
...
build 172 已上传成功。固定源码通过 1,270 次 Swift 测试执行、159 项 SDK 27 完整 App 回归及该 SDK 的 iPad/iPhone 手记 UI 用例；加急上传按用户要求跳过上传 SDK 的模拟器验收，Apple 处理和 Floe QA 可安装状态已于 2026-09-14 17:52:18 UTC 确认。APT 官方签名仓库仍未发布，不能把 PyPI/npm 可用性等同于完整 Linux 软件包生态。当前证据和限制见[反馈修复记录](FLOE_156_FEEDBACK_REPAIR.md)。
