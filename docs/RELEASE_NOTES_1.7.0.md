...
## Build 163 反馈修复候选（尚未分发）

- 手记改为封面入口与全屏编辑，可直接从 Floe 项目／聊天工作区导入文件并保存独立副本；全文搜索覆盖正文、有效的 Office 索引与逐页 OCR，显示命中片段及未完整索引的状态。
- 手记顶部返回与文档操作合为一行，手机次要操作收进菜单；Apple Pencil 捏压／双击遵循系统偏好，快捷菜单支持工具、颜色、粗细与撤销，硬件手势待真机确认。
...
上述为候选源码行为，构建号已准备为 163，完整 App 回归与新 TestFlight 可用状态尚未确认。APT 官方签名仓库仍未发布，不能把 PyPI/npm 可用性等同于完整 Linux 软件包生态。当前证据和限制见[反馈修复记录](FLOE_156_FEEDBACK_REPAIR.md)。
