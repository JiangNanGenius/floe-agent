...
实际 RunContext 读取最新 PlanDraft，但组装器此前将所有状态都称为“已接受计划”，还会静默省略第 9 段以后的工作、第 9 项以后的验收和每段 240 字以后的细节；假设与风险未传递。现按保存状态区分草稿与已接受计划，不再注入已归档/已替代的计划；传递全部有序段落、完整细节、验收、假设的接受状态和风险缓解措施。当前用户指令和模式继续优先，草稿不构成执行授权。

两项新回归覆盖 12 段计划、长细节、后续验收及六种状态，在普通和本地精简提示词路径均验证；HarnessPlanningTests 共 37 项通过。此修复选择保留完整要求，不能宣称减少实际 token；超大计划的预算/检索、实际最终模型请求和中途引导效果仍需继续验收。H01–H06 保持未完成，最终 Skill/工具说明审查仍是发布门槛。
