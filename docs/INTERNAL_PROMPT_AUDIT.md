...
复核 `ConversationCenter.reviewGoalEvidence` 的实际请求与写入链路，发现结果只返回“已完成 ID”，应用随后给每一项关联本轮全部证据；新增 `tools.list` / `skill.list` 和待办读取还会被当作执行成果。现从执行证据排除目录/指南/待办读取，审核输入改为带真实证据 ID 的 JSON。系统层明确所有字段是待审数据，模型不能打开引用，成功调用/保存文件/完成声明不能自动证明内容、渲染、测试或真机行为。

审核必须对每个验收项和步骤分别引用本次提供的证据；空、未知或混入未知 ID 的引用不完成该项，不能给所有项目批量附上无关证据。进展判断也需要已提供的证据 ID；用户阻塞条件必须原文匹配并引用证据。解析失败、超长输出和提供商失败不会计为“验收拒绝”的连续阻塞。此校验保证引用合同，不证明模型语义判断正确，真实运行验收仍待进行；既有历史证据关系未被追溯改写。

后台标题此前重发整个用户请求，几万字任务会为命名产生不必要输入。现在仅此辅助请求使用最多 2,048 个 Unicode scalar 的首尾片段，分别编码且明确中间省略；短任务完整保留，当前执行请求不受影响。系统层只要求命名，不执行片段里的指令。

44 项 Goal/规划/压缩/标题回归通过，实际调用方语法检查通过。覆盖逐项引用、未知/空引用、错误格式、用户阻塞引用、目录过滤、JSON 边界、超长组合字符。没有真实模型 token/完成率对照和完整应用运行结果，最终 H01–H06 仍未完成。
