...
| Floe guest runner | `/usr/local/bin/floe-exec` (injected) | this repository (`FloeAgent/LinuxGuest/`) | MPL-2.0 (repository license) | source in-tree; CI builds a static riscv64 binary and records its sha256 | none |
...
3. Feed the notices into the app's open-source acknowledgements path
   ...
   actually shipped).
4. Re-verify this manifest against the exact artifact that ships; a
   download list entry does not by itself satisfy license obligations.
