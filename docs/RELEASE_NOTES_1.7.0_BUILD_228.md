# Floe 1.7.0 (228) — candidate / 候选说明

**Status / 状态：源码候选，尚未完成云端 App 构建、上传、Apple 处理或真机验收。** This document records intended changes and verification boundaries; delivery evidence will be added only after each gate passes.
...
本候选将 IDE Office 标签接入现有共享文档会话的实际打开流程，并在失败时结束等待、给出重试。PPT 原生宿主对已加载且页面尺寸已确定、但首个 tile 尚未到达的文档提供有界的编辑入口后备；成功状态仍要求编辑后的真实绘制，不把页面尺寸误报成首帧。宿主修改必须经云端重建、固定产物和 App 打包检查。DOCX/XLSX 仅作防回归验证，现有 Office 路由和保存逻辑保持不变。
...
IDE Git 侧栏的窄宽度操作采用一致的触控目标。运行面板提供自动、单核、双核选择，并将请求交给 Linux 启动链。双核 `fork/exec` 已通过云端真实客体启动检查，但性能门槛 S5 的固定计时标记无效，需修正后重测；正式版继续以单核安全门拒绝显式双核请求，不静默降档。选择本地模型时提前执行与活跃 Linux 客体的冲突确认；取消不改变原有选择或停止客体。Qwen 多 token 预填充现由固定的 `mlx-swift-lm` 本地补丁走现有逐步运算路径，单 token 解码仍保留融合内核；Office 不再于启动时预热并安装全进程信号处理器。许可原文并入现有一份开源声明。MLX 加载与普通对话的设备级故障须另有实际运行证据，不能仅凭诊断或预加载通过宣称已修复。
...
This candidate opens IDE Office tabs through the existing shared document session and ends failed waits with a retry path. The PPT native host has a bounded edit-entry fallback for a loaded, sized document whose first decoded tile is delayed; readiness still requires a real paint after entering edit mode. A changed host must be rebuilt in cloud CI, pinned, and checked in the App payload. DOCX/XLSX are regression checks; existing Office routing and save behavior stay in place.
...
Narrow IDE Git actions use consistent touch targets. The Run sheet offers automatic, one-core and two-core choices and passes the request to Linux startup. Real dual-hart `fork/exec` passed the cloud boot check, but the S5 performance gate used an invalid fixed-time completion marker and needs a corrected rerun. The shipping single-core gate still refuses explicit two-core requests without silently downgrading them. Selecting an on-device model asks to resolve an active Linux conflict before changing selection; cancel keeps the previous model and guests. A pinned local `mlx-swift-lm` patch routes multi-token Qwen prefill through the existing per-step operations while retaining the fused single-token decode path. Office no longer prewarms at app launch and installs its process-wide signal handler before chat. Its MIT notice is included in the existing unified declaration. The MLX device failure remains open until actual loading and chat evidence proves a repair.
