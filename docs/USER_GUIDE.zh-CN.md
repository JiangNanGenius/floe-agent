...
- **Python 包无法激活：**Linux 环境内的 `pip` 安装的是客体真实 Linux 包，缺少 riscv64 兼容构建时会给出相应原因。由应用技能安装器安装的包仍限于纯 Python 通用 wheel。已退役的 iOS wheel 产线与原生扩展内置不再是 App 的一部分；仅支持原生扩展的包请在已信任的 SSH 主机上运行。
