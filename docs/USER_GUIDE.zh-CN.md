...
7. 普通任务的附件菜单可“添加手记资料”，默认只读，可显式允许编辑。助手的 `notes.read`、`notes.search` 与 `notes.edit` 只使用当前对话已选择的手记范围。

在“设置 → 通用”选择浅色、深色或跟随系统；同一页面内的“Whisper 与语音识别”管理约 491 MB 的按需多语言模型。Whisper 不可用时使用 Apple 识别及其系统权限。本轮尚未完成真实中英混合语音、长录音与所有媒体转录入口的统一验收。
