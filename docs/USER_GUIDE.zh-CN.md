...
工作区文件和 IDE 已接入本地只读 DXF、网格模型、Gerber／钻孔预览，可展开为全屏。DXF 可切换图层，三维模型可旋转、缩放和平移。具体格式限制和 App 验证状态见[工程查看器说明](FLOE_ENGINEERING_VIEWERS.md)。DWG、STEP、KiCad 的解析器仍待接入，暂不宣称全格式 CAD 支持。
