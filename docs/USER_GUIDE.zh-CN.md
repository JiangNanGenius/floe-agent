...
**源码状态——下一构建。** `main` 已领先于已交付的 225 构建，并包含下一轮集成：原生 IDE 文本/代码编辑器、本机压缩归档、不可变环境模板与私有磁盘、CPU/内存资源池、后台/画中画身份修复、本地模型的同任务 Linux 工具继续、展开式提示词编辑器与按会话草稿、唯一的第三方许可入口、原生优先媒体路由，以及下一次 Office 宿主固定。下文标记**下一构建**的条目均已集成到源码，并按各条目列出的检查（模块测试、云端 App/模拟器运行或宿主固定构建）验证；但它们**不在**已交付的 225 构建中，尚未分配新的版本号或标签，也不构成真机验收。[下一版状态](FLOE_1_7_NEXT_RELEASE_STATUS.md)。
...
**下一构建——原生 IDE 编辑器。**打开文本、代码或未知文本文件时使用原生 Swift 编辑面：标签栏多缓冲区、行号、有界语法高亮、支持环绕的查找替换、撤销/重做，以及带版本校验的保存与保留草稿的并排冲突审阅。草稿按缓冲区保存：切换标签或编辑器不会丢弃未保存内容。守卫规则明确而不静默——超过 4 MiB 文本上限或不是合法 UTF-8 文本时按原因回退；最多同时打开 12 个缓冲区，当 12 个都含未保存草稿时，新打开会被拒绝并给出双语说明与**全部保存**入口，而不是关闭他人的草稿。高亮刻意限制在约 512 Ki UTF-16 单元以内，更大文件仍以纯样式打开。原 Web 工作台保留为一个开关（**Web 编辑器**），继续提供多光标、折叠与自带文件浏览器，切换不会丢弃原生缓冲区。Office 与 PDF 仍使用各自表面，文件树、源码管理、终端与运行面板共用，运行仍限于本机运行时提供的语言。现有证据为模块测试加一次云端模拟器 UI 运行（iPad 的原生保存/冷启动重开、Web 回退、DXF 与 DWG 均通过）；紧凑宽度的工具栏钩子修复与四用例门禁已提交，其确认性模拟器复跑在撰写本指南时仍在进行，真机键盘/输入法行为仍待完成。
