...
画布是专注的编辑面，不是第二个不受限浏览器。选择**画布助手**后，它只可使用 `web.search` 和 `web.fetch` 研究公开资料，不能导航或点击浏览器、登录、调用终端/SSH/Git、任意读图或读取任意工作区文件；助手输入框支持与普通任务共用的本机语音听写。研究结果保持只读，只有用户点击**加入画布**后才会生成带来源 URL、Run 身份和“许可待确认”标记的画布文本。把公开网页图片作为生成参考时，Floe 会先下载、校验、去重并保存到素材库，再插入一个可见图片节点；不会把网页 URL 直接作为参考图交给生图服务。图片或视频生成会留下“提示词 → 生成配置 → 结果”节点链；双击配置节点可以修改参数或重试，失败节点也提供“重试”入口并保留失败记录。画布 Agent 每个用户轮次最多生成一次，供应商失败后不会自动循环。标准 MCP 默认仍不向画布开放。
