...
- **本地语言**：Build 196 内置的签名目录提供 Lua 5.4.8、Ruby 3.4.1 与 PHP 8.2.33（`apt install floe/<名称>`）；可安装性以该已验签目录为准，真机运行验收由测试者完成。PHP 8.2 处于安全维护期（至 2026-12-31），固定为已打补丁的 8.2.33。编译型语言走云端编译路线，暂存工件未签名。
