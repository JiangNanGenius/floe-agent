...
本指南适用于 Floe Agent 1.4.33 源码目标（build 64）。具体标签会随系统语言和服务商配置略有变化；分发状态仍须以 TestFlight 为准，源码标签本身不是 Apple 处理回执。
...
本地模型会获得与当前请求最相关、数量受限但真实可执行的工具。它们使用独立的对话长度和历史整理策略，不会影响云端模型的上下文或工具能力。本地模型无法正确提出工具操作时，Floe 会说明原因，不会伪装为已经执行。
