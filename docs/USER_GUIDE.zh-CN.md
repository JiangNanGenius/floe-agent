...
- **本地语言**：PHP 8.2.33、Ruby 3.4.1 与编译型语言正在云端验证，实际可安装状态以已签名并发布的能力目录为准。
...
Build 179 候选版还以签名 WASM 能力形式提供 Lua 5.4.8：在 Shell 中通过 `apt install floe/lua` 全应用安装，支持 `apt search`、`apt list`、`apt show` 和 `lua` 别名；同一条命令混用 WASM 与 Debian 包会在变更前被拒绝。全应用 WASM 能力与环境层软件包相互独立。Build 192 修复候选把 Ruby、PHP 作为 **compilepending 候选**加入：候选资产已通过宿主级测试，但没有签名工件，设备上不可安装；在签名目录出现对应条目之前，不要期待 `ruby`／`php` 别名。解释器启动可能超过 Shell 默认超时，请提高 `timeout` 或改用后台任务。Rust／Swift／C/C++ 不在本机编译，请按上文“运行当前代码文件”路由到已配置主机。
