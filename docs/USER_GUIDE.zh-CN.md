...
本指南适用于 Floe Agent 1.4.26 源码目标（build 57）。具体标签会随系统语言和服务商配置略有变化；分发状态仍须以 TestFlight 为准，源码标签本身不是 Apple 处理回执。
...
**数据管理 → 字体资源**保存一份按内容哈希去重的 Floe 全局字体，支持从“文件”导入或从公开 HTTPS 直链下载 TTF、OTF、TTC、OTC，单文件上限 32 MB。启动时 Floe 会重新注册这些字体，因此 Word、PDF 和所有工作区都能复用，不需要每个工作区重复下载。`font.list`、受限的 `font.install` 和 `font.resolve` 在自动模式下不等待审批模型；影响所有工作区的 `font.remove` 仍需审核。iOS 也允许 Floe 使用系统或字体提供 App 已安装的字体；受 Apple 公共 API 限制，任意网络字体不会被静默安装给 Floe 以外的其他 App。
