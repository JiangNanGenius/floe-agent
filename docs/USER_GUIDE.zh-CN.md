...
本指南适用于 Floe Agent 1.4.49 源码目标（build 80）。具体标签会随系统语言和服务商配置略有变化；分发状态仍须以 TestFlight 为准，源码标签本身不是 Apple 处理回执。
...
画布是专注的编辑面，不是第二个不受限浏览器。选择**画布助手**后，它只可使用 `web.search` 和 `web.fetch` 研究公开资料，不能导航或点击浏览器、登录、调用终端/SSH/Git、读图或读取任意工作区文件。研究结果保持只读，只有用户点击**加入画布**后才会生成带来源 URL、Run 身份和“许可待确认”标记的画布文本。标准 MCP 默认仍不向画布开放。
