...
本指南适用于 Floe Agent 1.4.45 源码目标（build 76）。具体标签会随系统语言和服务商配置略有变化；分发状态仍须以 TestFlight 为准，源码标签本身不是 Apple 处理回执。
...
画布是专注的编辑面，不是第二个不受限浏览器。需要联网素材时，回到所属工作区的普通任务，让 Agent 使用 `web.search` 和 `web.fetch`；画布路径不开放浏览器控制。Agent 可把结果保存进工作区，再由用户放进画布整理。标准 MCP 默认不向画布开放。

需要连接标准远程工具服务器时，进入**技能 → 标准 MCP**并添加 Streamable HTTP 地址。Floe 支持无需认证、Bearer Token 和自定义请求头；秘密值只进入钥匙串，不写入服务器元数据。每个服务器和发现到的工具都能单独启用或停用。远程工具使用服务器专属命名空间，继续受当前任务的本地权限与审批约束，并把服务器描述和返回内容视为不可信数据。打开**允许画布使用**只记录未来画布 Agent 工作流的明确选择，不会扩大普通任务权限，也不会绕过审批。
