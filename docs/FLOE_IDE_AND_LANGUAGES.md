...
Each IDE binds to the workspace file service present at opening. BrowserFS reads, directory operations and saves pass through `IDEWorkspaceSession` and the existing path/secret/symlink checks. Native atomic saves finish before the editor receives success. The first read establishes a SHA-256/mtime baseline. Concurrent Agent edits produce a conflict; repeated search reads cannot silently replace that baseline. Editing UTF-8 text is capped at 4 MiB per file. The bridge retains hashes plus a bounded 16 MiB original-text cache for three-way review. Closing with pending edits offers save, keep editing or discard; a failed save keeps the IDE open. After an external edit conflict, a native review combines independent text edits and asks about overlapping regions. The draft is retained in `Recovered Edits`; committing the reviewed result rechecks both the open editor draft and disk version. A changed disk version opens a new review. Missing/oversized baselines require an explicit whole-file choice. See [concurrent editing](FLOE_CONCURRENT_EDITING.md).
...
### Concurrent-edit checkpoint

Source `0fd21a32c3e06c75bc58a84f15b31ea92ffafc0e`, CI [34960853663](https://github.com/JiangNanGenius/floe-agent/actions/runs/34960853663): all **7 native filesystem tests** passed, including competing commits, repeated conflicts and deleted-original recovery. Artifact `10393446450` retains the platform log. The full App and dual-device UI phases are separate and remain pending at this checkpoint.

The pinned real CodeBlitz browser prototype applied a reviewed draft, rejected an older review, saved the result and reported clean state. Screenshot: [browser conflict integration](evidence/floe-1.7/ide/codeblitz-conflict-resolution-browser.png). The temporary buttons belong to a synthetic test fixture, not the shipping app UI; storage in this browser fixture is a Map, not native disk. The prototype initially exposed two upstream icon-theme CDN fetch errors. The integration now removes that pinned default contribution and uses bundled file icons; a repeat merge/reject/save completed with no unhandled errors. Arbitrary extension workers remain intentionally disabled.
