...
| Guest network, route and DNS on first boot | The guest runner (PID 1) now configures `eth0` with the slirp address/route through ioctls, writes `/etc/resolv.conf` (public resolvers; slirp's 10.0.2.3 is not served by the pinned engine and is deliberately not written), writes `/etc/network/interfaces.d/eth0`, and runs a bounded DNS probe before answering capability negotiation. The answer carries `net=up|partial|down`; the host parses and surfaces it and never treats a missing field as ready. | `FloeAgent/LinuxGuest/runner/floe_net.h`, `floe_exec.c` (`guest_bring_up_network`), `LinuxGuestService.swift` (`LinuxGuestNetworkStatus`), `LinuxGuestRegistry`/`LinuxGuestCommandChannel` |
