...
Supported builds bundle native NumPy, Pillow, and pandas; the runtime probe is authoritative for the current build, and additional pinned packages (regex, PyYAML, MarkupSafe, orjson, pydantic-core) ship through the ios-wheelhouse pipeline as they pass qualification. Use available native libraries directly. SciPy, scikit-learn and Matplotlib cannot ship natively on iOS (no Fortran toolchain / heavy native stacks); use the explicitly identified browser Pyodide route or a configured trusted SSH host. Report the actual execution environment and results.
...
Candidate build 166 adds document tabs: select a tab to switch, use × to close its view without deleting the document, and use + to find another document by title or indexed content. Returning to the library keeps open tabs. Relaunch still starts in the library. The arrow beside the writing toolbar collapses the document header and tab strip while leaving writing tools available. Each document retains its page, viewport and tool; a failed Office save prevents switching. Squeeze opens a circular tool wheel: pen, highlighter, eraser, lasso and AI selection occupy fixed positions. The current tool is highlighted; selecting one dismisses the wheel immediately, and the center cancels without changing tools. Color and width stay in the main writing toolbar. These new interactions await complete App qualification.
