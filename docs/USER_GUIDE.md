...
Native squeeze and double-tap callbacks follow the system action preference, using the hover location for the contextual palette when available. Disabled gestures and system shortcuts are left to the system; finger drawing remains opt-in. See [Apple's Pencil interaction API](https://developer.apple.com/documentation/uikit/uipencilinteraction). This is build 162 candidate source: dual-device UI and distribution are pending, and physical squeeze delivery requires compatible hardware.
