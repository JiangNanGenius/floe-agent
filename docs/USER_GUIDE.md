...
**Source status — Build 226 candidate, not yet delivered.** `main` is ahead of the delivered 225 build with the native IDE, compressed archives, reusable Linux templates and private disks, resource management, background/PiP and notification repairs, local-model tool continuation, multiline prompt editing with per-conversation drafts, one third-party license entry, native-first media routing, and the pinned Office host. Component, cloud App, simulator, distribution, and physical-device evidence remain separate gates; see the [Build 226 candidate notes](RELEASE_NOTES_1.7.0_BUILD_226.md). Entries marked “next build” below describe source behavior, not Build 225 or a completed device acceptance.
...
These screenshots show the Build 226 candidate in the iPad mini and iPhone simulators with a synthetic test workspace. They demonstrate the native layout and compact Explorer drawer; they are not physical-device acceptance.
