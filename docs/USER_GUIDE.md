...
Current internal delivery: **1.7.0 (227)**. Immutable tag `v1.7.0-beta.84` binds source `9c756864`; [release run 35957256008](https://github.com/JiangNanGenius/floe-agent/actions/runs/35957256008) retained the unsigned IPA (739,368,613 bytes; SHA-256 `a77b3b9a120a55dd6737bf1fb89efe7609c8917ca3facab7cd9cbb5c4c66b30c`) before signing, accepted the signed upload and published the [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.84). Apple build `640e39a2-001b-4672-9b17-b4a378d9eb6a` was verified `VALID`, unexpired and `IN_BETA_TESTING` in the sole private Floe QA group at 2026-09-24 05:42 UTC by [verify run 35961062720](https://github.com/JiangNanGenius/floe-agent/actions/runs/35961062720), and both test-note languages were read back. The matching unsigned GitHub developer IPA and Feather feed entry are published; on Gitee only the small release assets are mirrored, because the repository's attachment quota cannot hold the ~705 MiB IPA. [Build 227 notes](RELEASE_NOTES_1.7.0_BUILD_227.md) · [Delivery record](TESTFLIGHT_1.7.0_BETA.md).

**Source status — unreleased repairs after Build 227 (no new build announced).** `main` is ahead of the delivered 227 build with work-in-progress repairs that are not part of any delivered build: a bounded extent-bootstrap fallback for the PPT edit entry, IDE Office tab opening through the shared document session, touch-sized IDE Git sidebar actions at narrow widths, and Linux guest run-shape/core-choice plumbing. Component, cloud App, simulator, distribution and physical-device evidence remain separate gates; the feature scope carried by the delivered build is recorded in the [Build 226 candidate notes](RELEASE_NOTES_1.7.0_BUILD_226.md) and the [Build 227 notes](RELEASE_NOTES_1.7.0_BUILD_227.md). Entries marked “next build” below describe source work beyond Build 227 with no device acceptance yet.

**Build 227 device regressions (not accepted).** iPad testing of the delivered build on 2026-09-24 reported that a downloaded MLX local model crashes in ordinary chat and in the benchmark, that a PPT/PPTX preview opens but the edit entry then stalls, and that Word/Excel/PPT documents opened from the IDE file tree stay on the opening indicator. These are open regressions tracked for the next candidate — not fixed or waived. Local-model load and benchmark, PPT editing, PiP, notifications, keyboard/IME input and dual-core guest operation remain unaccepted on a device.
