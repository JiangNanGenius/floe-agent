...
- Existing conversations and the Canvas assistant can select Notes material, read-only by default. The picker displays and revokes grants; revocation does not remove text or Office attachments already sent to chat. `notes.read`, `notes.search` and `notes.edit` use only that conversation's selected scope. Direct selection on the new-task home screen is still pending.
