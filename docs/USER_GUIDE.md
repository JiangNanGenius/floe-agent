...
This guide describes the Floe Agent 1.4.27 source target (build 58). Labels may vary slightly with the system language and configured provider. Distribution status must still be verified in TestFlight; a source tag is not an Apple processing receipt.
