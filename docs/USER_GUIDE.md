...
Current internal delivery: **1.7.0 (225)**. Apple `VALID`, unexpired and `IN_BETA_TESTING` in the sole private Floe QA group were verified September 22 at 20:17 UTC, and both test-note languages were read back. The matching unsigned GitHub developer IPA and Feather feed entry are published; on Gitee only the small release assets are mirrored, because the repository's attachment quota cannot hold the 712 MiB IPA. Physical-device behavior remains for the tester. [Build 225 notes](RELEASE_NOTES_1.7.0_BUILD_225.md) · [Delivery record](TESTFLIGHT_1.7.0_BETA.md).

**Source status — next build.** `main` is ahead of the delivered 225 build and also carries the next integration: the native IDE text/code editor, native compressed archives, immutable environment templates with private disks and the CPU/RAM pool, background/PiP identity repairs, the local-model/Linux tool continuation, the expanded prompt editor with per-conversation drafts, the single third-party license entry, native-first media routing, and the next Office host pin. Items marked **next build** below are integrated in source and covered by the checks named at each entry — module tests, a cloud App/simulator run or the pinned host build — but they are **not** in the delivered 225 build, no new version or tag has been assigned, and none of them is physical-device acceptance. [Next-release status](FLOE_1_7_NEXT_RELEASE_STATUS.md).
...
The same Files inspector browses ZIP, TAR and 7z archives: entries are listed with size and bounded preview, and extraction is staged into a temporary hidden workspace directory that is removed when the browser closes. **Next build** adds on-device compressed tar (`.tar.gz`, `.tar.xz`) and single-file gzip/xz, plus archive creation: the Agent can create ZIP, TAR, tar.gz, tar.xz, tar.bz2 and single-file gzip/bzip2/xz archives, and list or extract ZIP, TAR, tar.gz, tar.xz and 7z, all within bounded entry, size and cancellation limits and without starting a Linux guest. bzip2 *decoding* and browsing RAR are refused with their concrete reason (bzip2 creation still works; RAR needs the app's signed decoder, so ask the Agent to extract it). A format this app cannot read says so instead of silently starting a guest or failing with a generic error.
...
- **Files** for the current workspace tree, including ZIP, TAR and 7z archives that open in a bounded browser with entry preview and staged extraction. **Next build** adds on-device tar.gz/tar.xz and single-file gzip/xz handling and archive creation; bzip2 decoding and RAR browsing report the concrete reason instead of starting a guest;
...
**Next build — the native IDE editor.** Opening a text, code or unknown text file from the workspace or the IDE now uses a native Swift editing surface: multiple open buffers in a tab strip, line numbers, bounded syntax highlighting, find/replace with wrap, undo/redo, and save with version checking and a side-by-side conflict review that keeps your draft. Drafts stay per buffer: switching tabs or editors does not discard unsaved text. Guards are explicit rather than silent — a file over the 4 MiB text limit or without valid UTF-8 text falls back with a stated reason, and up to 12 buffers stay open; when all 12 hold unsaved drafts, a new open is refused with a bilingual explanation and a **Save All** action instead of closing someone's draft. Highlighting is deliberately bounded (about 512 Ki UTF-16 units) and larger files still open with plain styling. The previous Web workbench remains one switch away (**Web editor**) for multi-cursor, folding and its own explorer, and the switch never discards native buffers. Office and PDF keep their own surfaces, the file tree, source control, terminal and Run panel are shared, and Run stays restricted to the languages the local runtime offers. The checks are module tests plus a cloud simulator UI run (iPad: native save/cold-reopen, Web fallback, DXF and DWG all passed); the compact-width toolbar-hook fix and the four-case gate are committed, its confirming simulator re-run was still in progress when this guide was written, and physical-device keyboard/IME behavior remains open.
...
The PowerPoint repair on top of Build 219 fixes a presentation-only edit failure: the mobile editor starts every presentation in its endless-slide viewing layout, and the previous automation refused the engine's own edit entry for that layout, so a PPTX could report itself editable while the engine stayed in the viewing UI. Floe now follows the engine's guarded mobile entry for presentations and drawings and verifies that the engine actually switched into its editing UI before any writable claim is made. Readiness is also render-truthful: a presentation (PPT, PPTX, PPTM, PPS(X), POT(X), ODP/OTP/FODP, ODG/OTG/FODG) is not "ready" until the host has decoded a real document tile on a sized canvas. If no tile appears before the bounded deadline, the editor shows an actionable failure with retry/recovery instead of a blank ready editor, and the editing copy is retained. Word and Excel documents keep their existing open-only readiness. The delivered build's host was compiled and re-pinned from the `office-native-host` run 35668651442 on 2026-09-22; **next build** pins the newer cloud host from run 35818562658 (commit `912e3b59`, post-entry paint validation, with verified archive, executable and resource digests) so the App builds against the contract that gates the edit entry on a real paint. The device-capability receipts (embedded editor, visible render, device round-trip and original-file write-back) remain unproven, and the physical-device presentation, save, close and reopen checks remain with the user.
