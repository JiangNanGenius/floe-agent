...
The canvas is deliberately a focused editing surface, not a second unrestricted browser. Choose **Canvas Assistant** to research public references with only `web.search` and `web.fetch`; it cannot navigate or click a browser, log in, use a terminal/SSH/Git, inspect arbitrary images, or read arbitrary workspace files. The assistant composer supports the same on-device dictation control as a normal task. A research result remains read-only until you choose **Add to canvas**, which creates a note carrying its source URLs, Run identity, and an unverified-license marker. When a public web image is used as a generation reference, Floe first downloads, validates, deduplicates, and stores it in the material library, then inserts a visible image node; raw web URLs are never passed to the image provider as reference inputs. Image/video generation creates a visible prompt → configuration → result chain. Double-click the configuration node to edit or retry it; failed nodes also expose a Retry action and preserve the failed attempt. A Canvas Agent performs at most one generation attempt per user turn and never loops on a provider failure. Standard MCP remains disabled for canvas by default.
