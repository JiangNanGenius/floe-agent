...
Do not submit this draft until the actual build and review access have been confirmed. Replace the bracketed fields; credentials must be supplied privately in App Store Connect.

App: Floe Agent, `org.floeagent.ios`.
Submitted version/build: [FINAL VERSION / BUILD].
Contact and model access: [CONFIRMED REVIEW CONTACT AND PRIVATE ACCESS INSTRUCTIONS].
...
3. Open Settings → Models and Providers and follow the private model configuration supplied with this submission. Return to New Task, ask a short question, then request a small document from the supplied sample content. Inspect tool progress and cancel a subsequent operation.
...
Before submission, attach the actual working model-access steps and synthetic sample files, verify each walkthrough on the selected build, and replace any navigation wording changed in that build. Keep required services accessible throughout review. These review-access requirements follow [Apple's submission guidance](https://developer.apple.com/app-store/review/guidelines/#before-you-submit).

Image generation is an optional integration that requires the user to configure a compatible service. No image-generation credential is included in this proposed review setup. The core walkthrough uses Notes, documents, chat and tool execution. Confirm the final access arrangement before submission; if review requests additional access, respond explicitly rather than claiming this draft establishes review completeness.
