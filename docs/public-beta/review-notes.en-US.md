...
4. In the Notes library, check actual Word, Excel, PowerPoint and supported CAD content covers. A labelled summary is a fallback and is not an original-layout preview. Open, rename, close and reopen the same file.
5. If testing cloud compilation, connect your own GitHub account, open source in the IDE, select GitHub Actions, review the source snapshot and explicitly select a repository/workflow. After dispatch, close and reopen the App: persisted runs should reappear and update automatically. Closing the App does not cancel GitHub; cancellation waits for confirmation. GitHub runner output targets Linux/macOS, not iOS executable installation. Installing a workflow template is a separate explicit repository operation.
6. Open Creative Mode to create and organize a canvas. Access to remote computers, personal accounts and system permissions is optional; review does not require access to a reviewer's private machine or data.
