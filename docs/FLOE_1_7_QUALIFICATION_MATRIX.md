...
上表 `floe/model-whisper-small` 是通用模型目录的候选 ID；当前应用另有 WhisperKit 固定资源通道，不能混淆两者的安装状态。固定来源与摘要的约 491 MB Small 多语言资源已在运行 `34728314865`、提交 `d51cad7ff516657a1f109e83a6eefbdd805df127` 上完成 iPad/iPhone SDK 27 模拟器真实 Core ML 推理，识别约 9 秒合成中英样本且未回退 Apple。证据在 [Whisper 目录](evidence/floe-1.7/whisper/)。真人混说、完整麦克风/文件入口及真机峰值内存尚未验收；此结果不代表其余 32 个候选模型可用。
