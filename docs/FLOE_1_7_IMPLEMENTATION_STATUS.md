...
Current internal delivery: **1.7.0 (227)**, immutable tag `v1.7.0-beta.84` binding source `9c756864fa9532d02ba7f7073320f8b48d94dc51`. [Release run 35957256008](https://github.com/JiangNanGenius/floe-agent/actions/runs/35957256008) compiled with Xcode 26.6, retained the unsigned `Floe-Agent-1.7.0-build227-unsigned.ipa` (739,368,613 bytes; SHA-256 `a77b3b9a120a55dd6737bf1fb89efe7609c8917ca3facab7cd9cbb5c4c66b30c`) before signing, accepted the signed TestFlight upload and published the [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.84). Apple build `640e39a2-001b-4672-9b17-b4a378d9eb6a` was verified `VALID`, unexpired, attached only to the private internal Floe QA group with no public link, and `IN_BETA_TESTING` at 2026-09-24T05:42:18Z by [verify run 35961062720](https://github.com/JiangNanGenius/floe-agent/actions/runs/35961062720); [prepare run 35961014056](https://github.com/JiangNanGenius/floe-agent/actions/runs/35961014056) read back the `en-US` and `zh-Hans` beta notes. Build 227 carries the Build 226 candidate scope (Runtime v2 templates/pool, native IDE, archives, prompt editor, one third-party license entry, GitHub-first downloads) plus the `FloeTools` import repair that unblocked the App compile; Build 226 itself never produced an IPA. Open Build 227 iPad regressions are unfixed: a downloaded MLX local model crashes in ordinary chat and in the benchmark, a rendered PPT/PPTX preview stalls at the edit entry, and Word/Excel/PPT documents opened from the IDE file tree stay on the opening indicator. Physical-device acceptance otherwise remains with the user. See [delivery record](TESTFLIGHT_1.7.0_BETA.md).

## Integrated source vs remaining gates (updated 2026-09-24) — implemented vs pending

`main` is ahead of the delivered Build 227 with unreleased post-227 repairs: IDE Office tabs resolve and open through the shared document session (`14c6e8b3`), the PPT edit entry has a bounded extent-bootstrap fallback with the Office host rebuilt and pinned by cloud run 36000058922 (`464e711e`, `1608e99e`, `2190669e`), narrow IDE Git sidebar actions use consistent touch targets (`812beda1`), and the guest core choice is carried through Linux startup behind the frozen single-core release gate (`9ec359fb`, `7e21d694`). No cloud App build, IPA, upload, Apple record or device retest exists for that source, and the next build number reserved in `project.yml` is not a delivery. The table below records what Build 227 shipped plus the checks and open device regressions; each row is covered only by the checks named. Work-package gates remain in [next-release status](FLOE_1_7_NEXT_RELEASE_STATUS.md).

| Area | Implemented and checked | Not verified / pending |
| --- | --- | --- |
| Native IDE | Native Swift text/code workspace is the default for text, code and content-verified unknown text files: tabs, line numbers, bounded highlighting, find/replace, undo, IME-composition guard and version-checked saves; the Web workbench stays mounted as the advanced fallback behind the `workspace.ide.nativeTextEditor` switch; 12-buffer budget with clean-only eviction and a bilingual refusal + **Save All** when all buffers are dirty; 4 MiB text limit. Module tests (`FloeWorkspaceTests`) plus cloud simulator run 35840760130 (`88603e9a`): iPad leg 4/4 (native save/cold-reopen, Web fallback, DXF, DWG). | The earlier step gate failed on a stale expected-case set and the iPhone leg (compact toolbar hook); the toolbar fix and the four-case gate are committed (`595b922c`), and the later targeted cloud App/UI run [35947162133](https://github.com/JiangNanGenius/floe-agent/actions/runs/35947162133) passed overall — iPad mini 4/4, with an iPhone DXF preview assertion that failed once and passed on automatic retry (the failure is retained). Real-device keyboard/IME/touch behavior is untested. On the delivered Build 227, Office documents opened from the IDE file tree stay on the opening indicator; the post-227 source opens them through the shared document session (`14c6e8b3`) with no App build or device retest. The native pane has no multi-cursor, folding or symbols/outline (Web owns those). |
| Archives | Host-native zip/tar/tar.gz/tar.xz create/list/extract; bzip2 create-only; 7z read-only; RAR list/extract through the app's signed decoder; entry/size/cancellation bounds, traversal/symlink containment and staged commit. Module tests (`ArchiveEngineTests`, `ArchiveToolTests`, `ArchiveBrowserServiceTests`, `HostArchiveBridgeTests`) plus 132 checks after the bounded-decode and tar-strictness repair. | bzip2 decoding is refused by design (its decoder cannot be bounded before allocation); a bounded native bzip2 decode is a queued follow-up. The optional guest `floe-host archive` bridge has no app-side negotiation (nothing answers `FLOE-HOSTREQ`) and there is no per-environment archive FileTree UI; no device automation covers the archive browser. |
| Environments/ templates/ pool | Layer stack (session → project → shared → base with an explicit write layer) and Runtime v2 immutable templates: content-addressed disks, pinned version/digest, per-environment deltas bound to the boot base, GC/quarantine rules and fail-closed pin errors; three-axis pool admission writes the granted shape into the TinyEMU boot config; shape changes linearize against stop; live-lease recovery, blob GC staging and failed-capture release fail closed. Module tests (`RuntimeV2TemplateTests` 17, `RuntimeV2StartupTests` 16, pool/channel suites, B3 12 deterministic races). Post-227 source adds the IDE Run sheet's automatic/one-core/two-core guest shape choice (`9ec359fb`, `7e21d694`), checked only by focused tests. | No App flow creates a pinned environment (current environments boot the verified base image); official `basic`/`dev-document` templates report `dependency-missing` until their installed-disk and recipe artifacts land; three cross-restart repair items (repair persistence and active-lease salvage) remain open. The frozen single-core release gate allows exactly one hart, so dual-hart requests fail closed with an actionable reason and a 2-vCPU default is not enabled; the six-vCPU tier stays dormant behind its evidence gate; the post-227 Run sheet choice reports the effective granted shape but an explicit two-core request is still refused (never silently downgraded) and has no App build or device retest; host cache directories are unpopulated beyond blob staging. |
| Background/PiP | Real per-launch runtime identity and supervised local-service lifecycle wired into the durable notification pipeline; measured-only metrics with "暂无"/"N/A" for unknown values; honest target existence and route prompts; generation-owned deferred routes; PiP snapshot compile repair. Component/unit suites (`FloeExecution` 202 with the known D-owned failures, `FloeCore` 229 tests/32 suites). | H5 repairs pending: stale-generation sample application, the `.bufferingNewest(32)` lifecycle stream dropping events, and a start/stop race that can publish a dead service handle. No device PiP or cold-launch notification acceptance. |
| Local model ↔ Linux | An idle-but-retained MLX engine is physically unmapped to admit a Linux start and reloads its snapshot on the next turn; genuinely active inference answers `.retained`, keeps the guest queued and fails truthfully after the bounded wait; switching models asks and releases the previous model on load; a run's own transient tool guest is released for its continuation (`b7fed4a5`: real arbiter+registry integration with a nonempty probe, transient-ownership and continuation suites). Module tests (`FloeLocalModels` 111 tests/18 suites, FloeCore arbiter 20/20). | Device acceptance of the local-model tool loop; the Build 225 iPad local-model load root cause remains unresolved without a failure log; on the delivered Build 227, iPad testing reports MLX crashes in ordinary chat and in the benchmark — an open regression, not a pass — and device memory-pressure behavior is unverified. |
| Prompt editor | Multi-line composer with a full-height editor bound to the same draft, durable per-conversation/Home drafts, Cmd-Return send with a composition guard, and complete-draft restore after a failed send. Simulator harness checks (57 and 63 across revisions). | Real-device IME, wrap, selection and long-prompt behavior are not device-tested. |
| Office PPT | Pinned cloud host from run 35818562658 (commit `912e3b59`) with post-entry paint validation; archive, executable and resource digests verified; the edit entry is gated on a real decoded presentation tile. Build 227 shipped this host; iPad testing reports the preview renders but the edit entry then stalls. Post-227 source adds a bounded extent-bootstrap fallback (`464e711e`, `1608e99e`) and a rebuilt host pinned by cloud run 36000058922 (`2190669e`) — unreleased and unverified. | Device-capability receipts remain false: embedded editor, visible render, device round-trip and original-file write-back; Build 227 did not pass PPT edit on a device; physical presentation edit/save/reopen stays with the user. |
| Downloads | GitHub-first Linux component download with fail-closed mirror switching, verified shard resume and a whole-archive SHA-512 recheck before import. | Gitee is explicitly partial: for Build 227 it mirrored the release source and six small assets, but the ~705 MiB IPA exceeds the repository's 1 GiB attachment quota, so no Gitee IPA exists for direct installation or as a Feather/AltStore source. |
