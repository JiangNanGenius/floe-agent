...
## Next integration on `main` (2026-09-23) — implemented vs pending

`main` is ahead of the delivered Build 225. The items below are integrated in source and covered only by the checks named; no new build number, tag, TestFlight upload or device acceptance is claimed. Work-package gates remain in [next-release status](FLOE_1_7_NEXT_RELEASE_STATUS.md).

| Area | Implemented and checked | Not verified / pending |
| --- | --- | --- |
| Native IDE | Native Swift text/code workspace is the default for text, code and content-verified unknown text files: tabs, line numbers, bounded highlighting, find/replace, undo, IME-composition guard and version-checked saves; the Web workbench stays mounted as the advanced fallback behind the `workspace.ide.nativeTextEditor` switch; 12-buffer budget with clean-only eviction and a bilingual refusal + **Save All** when all buffers are dirty; 4 MiB text limit. Module tests (`FloeWorkspaceTests`) plus cloud simulator run 35840760130 (`88603e9a`): iPad leg 4/4 (native save/cold-reopen, Web fallback, DXF, DWG). | The run's step gate failed on a stale expected-case set and the iPhone leg (compact toolbar hook); the toolbar fix and the four-case gate are committed (`595b922c`), and its confirming simulator re-run 35845688626 was still in progress when this record was written. Real-device keyboard/IME/touch behavior is untested, and the native pane has no multi-cursor, folding or symbols/outline (Web owns those). |
...
| Office PPT | Pinned cloud host from run 35818562658 (commit `912e3b59`) with post-entry paint validation; archive, executable and resource digests verified; the edit entry is gated on a real decoded presentation tile. | Device-capability receipts remain false: embedded editor, visible render, device round-trip and original-file write-back; physical presentation edit/save/reopen stays with the user. |
| Downloads | GitHub-first Linux component download with fail-closed mirror switching, verified shard resume and a whole-archive SHA-512 recheck before import. | Gitee is explicitly partial: the 712 MiB IPA is not hosted (repository attachment quota measured at 1 GiB), only small release assets are mirrored, and no Gitee IPA or Feather/AltStore source exists. |
