...
[CI 34951710478](https://github.com/JiangNanGenius/floe-agent/actions/runs/34951710478), source `1d86b285`, finished with **175 App regression tests passed** across 13 suites, accepted-SDK compatibility and Linux builds passed. Real package tests installed/imported/upgraded/uninstalled Python dependencies in a selected environment, reused bundled native lxml for a Chinese DOCX save/reopen, and installed/executed packages through both npm and pnpm. HTTPS requests and owned Python background services passed. These simulator results do not establish arbitrary native-addon compatibility.

The same run failed its Notes UI phase: iPad tabs/body search and Pencil/focused-layout tests passed; the imported-PDF test blocked in XCTest's broad accessibility query although the preceding snapshot contains the page. iPhone was not reached. [Diagnostic summary](evidence/floe-1.7/build172-repair/ci-1d86-notes-ui.json) and [actual iPad screenshots](evidence/floe-1.7/build176-ipad/) are retained. The follow-up narrows that query to ScrollView and adds bounded per-device diagnostics while preserving individual assertions/timeouts. This is a proposed test-query repair, not a claimed successful rerun.

[CI 34957110743](https://github.com/JiangNanGenius/floe-agent/actions/runs/34957110743), source `24b17c11`, includes the new CodeBlitz/native-files bridge and WASI changes. Its early qualification passed: 3 native IDE filesystem tests, 10 WASI command tests (including actual cwd-dependent file access), 66 other platform tests and 24 Notes storage tests. The subsequent App builds failed because the new IDE WebKit reply callback lacked SDK 27’s `@MainActor @Sendable` closure annotations. This is a source compilation failure, not an IDE UI pass. The annotations are corrected in the next patch. Run 34958886514 was cancelled after confirming the same known failure affected its source, avoiding a redundant heavy build. App/WebKit and dual-device UI remain pending.
...
## Concurrent human and Agent edits — 2026-09-15 implementation

Editing sessions stay available to both parties. Only the managed filesystem commit is briefly serialized, so SHA comparison and replacement cannot interleave across managed text/Office writers. Arbitrary native scripts must cooperate; this is not a kernel-wide file lock.

The ordinary text editor and CodeBlitz retain the opened version. A stale save preserves a content-addressed recovery copy and opens a three-way review: independent changes are combined, overlapping regions require a choice, and the combined result can be manually edited. Saving rechecks the reviewed disk version; a newer Agent edit requires another review. Deleted or unreadable originals leave the recovery copy intact and are not recreated automatically. Base retention and diff work are bounded; missing/large bases fall back to an explicit whole-file decision.

Local evidence: four BrowserFS/native-bridge tests pass; the real Swift merge implementation passes 114,310 independent-edit combinations plus overlap, insertion, deletion and CRLF checks. Seven native workspace tests passed in CI 34960853663, source 0fd21a32; full App conflict interaction remains pending. A subsequent patch adds Office immutable-version comparison/export and Notes per-object rebase, durable conflict copies and review. Office snapshot and Notes overlap/deletion/history tests are added; those newer changes still require cloud compilation and UI qualification. Do not describe this checkpoint as full collaboration qualification.

## 2026-09-15 engineering viewer addition

User added 3D/PCB/KiCad and common CAD read-only viewers to the current delivery scope. First patch connects local DXF, mesh and Gerber/Excellon viewers to workspace files/IDE with full-screen expansion; [matrix/evidence](FLOE_ENGINEERING_VIEWERS.md). Four fixtures × wide/compact desktop browser checks passed after correcting a real collapsed DXF surface. Core/test typechecking passed; cloud execution and new App UI tests pending. KiCad/DWG/STEP/IGES and other extra decoders are explicitly unbundled, not silently marked complete. CI 34966298524 still qualifies prior source 08ecacbf; do not attribute its results to the viewer patch or cancel it before retaining useful outcomes. Prior language/runtime/package/log-server/Qwen/release tasks remain unfinished.

## 2026-09-15 DXF/DWG editing and drawing review

Latest scope adds real DXF and DWG editing while keeping 3D/PCB read only. The MPL acadrust 0.5.5 worker now retains the native document, performs typed line/circle/text edits with undo/redo, verifies same-format saved bytes, and hands them to a SHA-checked native commit that keeps original versions and conflict drafts. AI review captures the real viewport and bounded parsed geometry/layer/source information, with an explicit user question through existing model services. No paid calls were repeated.

Exact engine `b82850f42510a43a27058b81bf466f0fd4a5635e`, run 34974090590: native three tests (six format/version round trips) + bounded WASM build + source/license inventory passed. Four latest-engine browser edit/save/reopen cases passed; two DWG outputs independently passed LibreDWG entity/text/geometry assertions. Browser native save was mocked. See [current matrix and public component evidence](FLOE_ENGINEERING_VIEWERS.md).

Retained prior full-App failure 34966298524 (`08ecacbf`): SDK compatibility/Linux/App compilation passed; native App regression had one WebKit load timeout out of 178 tests, iPhone Notes passed three flows with native Office skipped, iPad assistant restart timed out, iPad IDE launch timed out, and iPhone Files navigation used the wrong accessibility element below the fold. Current fixes refresh only the newly created assistant record, serialize the WebKit regression suite, and navigate the actual compact Settings collection. Assertions are not waived. New native CAD save/cold-reopen/review UI tests and binary conflict tests await the fresh cloud run.

No newer TestFlight upload, GitHub prerelease or Feather delivery is claimed. Internal 172 remains the last confirmed installable build. Runtime/package/language, KiCad/STEP/multilayer PCB, Qwen device acceptance, remaining review material and release/main-merge work retain their recorded status.
