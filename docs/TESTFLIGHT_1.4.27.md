# Floe Agent 1.4.27 (Build 58) TestFlight Notes
...
- **Data management:** Open **Settings → Data Management** and compare the total, app bundle, user data and safe-cleanup sizes. Open Archived Tasks, restore one item with a swipe, permanently delete one with confirmation, then test selected restore/delete. Safe Cleanup must report reclaimed bytes without removing workspaces, documents, models, fonts, attachments, the database or credentials.
- **Fonts:** In **Data Management → Font Resources**, import a TTF/OTF from Files and add another from a direct public HTTPS URL. Confirm duplicates are reused, the font remains available after relaunch and can be used from a different workspace. `font.list` and bounded `font.install` should not wait for approval-model review; `font.remove` must remain reviewed. If iOS does not expose a requested system font, Floe should explain that boundary and use an allowed managed font instead. Private/LAN URLs, credential URLs, non-font files and files larger than 32 MB must fail clearly.
