...
当前交互基线（1.5.0 与本轮开发分支）：画布使用“内容节点 → 任务节点 → 产物节点”的单一链路。生成上下文只沿显式来源关系读取，普通连线、产物关系和未连线备注不会进入提示词；保存配置、开始生成和失败重试共用同一任务快照，既有任务不会补造提示词节点。节点下方 AI 是无工具的节点原位编辑器，右侧 Canvas Assistant 才负责跨节点研究和编排。生成任务必须先保存配置，再由用户从任务卡明确开始；状态同时显示在任务卡和产物卡，失败重试复用原节点。空白处单指平移、节点单指移动、双指随时平移缩放、Pencil 绘制，连接端口支持实时曲线、磁吸和拖到空白处快速新建。
...
创意模式直接提供私人画布入口，普通绘图和已有内容编辑不依赖生图模型配置。iPhone 侧栏选中创意模式后收起，折叠画布导航直接进入编辑页；工具收进紧凑菜单，底部工具可滚动。新节点使用实际视口中心，尺寸变化保持中心，面板按可用空间约束。生成任务在所有创建菜单中列第一。

SVG/HTML/Markdown 原位模型编辑传递完整有界源内容并核对版本；冲突和保存失败不显示成功。前台版本核对补偿漏通知，交互草稿期间延后载入。详细范围与截图见[本轮升级](WORKFLOW_UPGRADE.md)。
...
- `CanvasProject`：可独立创建私人画布，也可绑定一个 Workspace；每个 Workspace 保留唯一的画布项目，项目内管理多张 `CanvasDocument`。
