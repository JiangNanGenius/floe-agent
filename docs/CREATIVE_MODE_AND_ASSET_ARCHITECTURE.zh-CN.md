...
当前交互基线（v1.4.73）：画布使用“内容节点 → 任务节点 → 产物节点”的单一链路。节点下方 AI 是无工具的节点原位编辑器，只以节点最新内容、保存配置和显式引用作为上下文；右侧 Canvas Assistant 才负责跨节点研究和编排。生成任务必须先保存配置，再由用户从任务卡明确开始；状态同时显示在任务卡和产物卡，失败重试复用原节点。空白处单指平移、节点单指移动、双指随时平移缩放、Pencil 绘制，连接端口支持实时曲线、磁吸和拖到空白处快速新建。
