...
- **Bundled Python:** `dis` and `_opcode` must import in the signed app. A declarative `pip install package==version` request must enter package-model review before download, then use the same quarantined pure-Python-only installer as the managed `packages` argument.
