...
- 手记合并同一文档版本的预览请求，系统超时后不再反复取消重启；保留总时限和可区分的内容摘要兜底。
- 修复 Office 摘要读取扩展名缺失的资源，以及编辑器不可用时缺少返回入口的问题。
- 组件验收保留 iPad、iPhone 各自结果；首次设备失败不再阻止另一端执行。

- 修复合法 Shell 导出变量超过旧上限时 Lua 无法运行的问题；保留分项大小限制和不含敏感内容的错误诊断。
- 修复预览 HTTP 分段请求与 CAD 封面就绪等待；取消和超时都能结束等待。
...
- Coalesce Notes previews for the same document revision and stop cancellation/restart storms after system timeouts, with a bounded budget and labelled summary fallback.
- Fix Office summary resource staging and retain navigation when the native editor is unavailable.
- Preserve both iPad and iPhone component outcomes even when the first device fails.

- Fixed Lua rejecting legitimate shell export sets above the old limit, with bounded input validation and content-free diagnostics.
- Fixed fragmented preview HTTP requests and bounded CAD viewer readiness, including cancellation and timeout.
