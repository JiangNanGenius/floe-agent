...
The Notes conflict review queue is currently session-local; the recovery documents survive restart and remain visible in the library. Recent baseline lookup is limited to 32 history rows. An unavailable baseline fails without guessing; the originating editor keeps its draft. Long-term persistent review metadata and full historical cross-document merging are not claimed.
...
- Added Office current-version snapshot and Notes independent/overlapping/deleted-page/baseline-history tests await the next cloud source. Native conflict sheets, iPad/iPhone interactions, Office rendering and restart recovery require full-App verification.
