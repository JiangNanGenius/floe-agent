...
Dispatched sources-only run against image run 35928029851 (fill in after the
run completes):

- Collector run: _pending_
- `linux-guest-sources-pypi-<run>`: wheels/ + sdists/ + pdfium-source/
- `pypi-source-gaps.tsv`: _pending_ (expected header-only)
- `SOURCES-MANIFEST.md` section 5 `gap_packages`: _pending_ (expected 0)
