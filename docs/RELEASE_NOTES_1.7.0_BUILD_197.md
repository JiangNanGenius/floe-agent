# Floe Agent 1.7.0 (197) / beta.54 — 发布候选（标签尚未创建）/ release candidate (tag not yet created)

> **状态：候选草稿，不构成交付。** Build 197（`MARKETING_VERSION` 1.7.0，
> `CURRENT_PROJECT_VERSION` 197）是下一内部测试候选，预留标签 `v1.7.0-beta.54`
> **尚未创建**。App 云端编译、TestFlight 上传、Apple 处理与真机验收均未开始；
> 本文只记录已实现行为、已通过的轻量校验与明确的待验证项。发布工作流将在包含本文件的
> 冻结提交上创建该标签并执行单次验收上传 SDK 构建（与 Build 196 相同流水线；标签创建后
> 不可移动）。
>
> **Status: candidate draft, not a delivery.** Build 197 (version 1.7.0, build 197) is
> the next internal-testing candidate; the reserved tag `v1.7.0-beta.54` has **not been
> created**. No App cloud compile, TestFlight upload, Apple processing or device
> acceptance has happened yet. This document records implemented behavior, the light
...
`codex/build192-feedback-round2`，8 个功能提交 / 8 implementation commits；最终冻结提交另记于
发布证据）。内部 TestFlight、GitHub
...
- `swiftc -parse` 解析本轮触及的 Swift 源文件；
- Office 嵌入适配器 2 项检查（`FloeAgent/scripts/test_office_embedded_controls.py`）；
- Office ink bridge 4 项检查（`FloeAgent/scripts/tests/test_office_ink_bridge.py`）；
- 只读权限 6 项夹具（`FloeAgent/scripts/tests/run_readonly_permission_fixture.py`）；
- notes bridge 5 项（`FloeAgent/scripts/tests/notes_bridge.test.cjs`）；
- Office mode-memory 独立 smoke（直接编译纯类型与轻量测试入口）；
- xcodegen 项目生成（`FloeAgent/project.yml` ↔ 已提交的 `FloeAgent.xcodeproj`）。

**未执行**：App 目标云端编译、模拟器／真机 UI、真实供应商调用（含方舟视频付费提交）、
TestFlight 上传、Apple 处理核验。App 云编译与用户真机验收待后续；本地 Qwen/GDN 首次消息
崩溃仍待真机确认，尚未证实修复。本文不替代冻结源码后的云端构建与真机证据。
...
Only these light checks were run:

- `swiftc -parse` on the Swift sources touched by this round;
- two Office embedded-controls checks
  (`FloeAgent/scripts/test_office_embedded_controls.py`);
- four Office ink-bridge checks
  (`FloeAgent/scripts/tests/test_office_ink_bridge.py`);
- the six-item read-only permission fixture
  (`FloeAgent/scripts/tests/run_readonly_permission_fixture.py`);
- 5 notes-bridge items (`FloeAgent/scripts/tests/notes_bridge.test.cjs`);
- the standalone Office mode-memory smoke (direct compilation of the pure type and
  lightweight test entry);
- xcodegen (`FloeAgent/project.yml` ↔ the committed `FloeAgent.xcodeproj`).

**Not run**: App-target cloud compile, simulator/device UI, real provider calls
(including paid Ark video submission), TestFlight upload, or Apple processing
verification. The App cloud build and the user's device acceptance remain pending; the
