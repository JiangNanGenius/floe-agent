...
| 类型 | 当前证据与限制 |
|---|---|
| Linux 客体运行时 | TinyEMU RV64 客体承载本地 Python/Node/Shell；环境磁盘按 APFS 克隆后稀疏、只增长到逻辑 8 GiB，开机在客体内幂等 `resize2fs`，旧盘原地迁移并记录容量来源；临时/缓存目录改到 `/floe/env`（pip/XDG/npm/TMPDIR），源码构建不再挤占根分区；9P 删除空目录/符号链接与 xattr 响应已修复（主机 9P 检查通过）。以上为单元/主机证据，磁盘扩容与真实 apt/pip/npm 的完整真机行为仍由云端 TinyEMU 资质与用户验收 |
| Linux 组件状态 | 设置页与终端共用一个权威状态（下载/进度取消/已装停止/运行中/修复/更新）；已安装或运行中的客体不会再显示“下载并启动”卡片；首次使用由共享可取消任务自动准备，并发入口只产生一次下载 |
