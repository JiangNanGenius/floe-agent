...
Current candidate: **build 167**, replacing the popover host with a thin open tool arc placed directly over the page, clamped inside its visible bounds. Squeeze toggles it, explicit tool taps select, and blank taps dismiss without changing tools; movement and lifting never commit. The final production component is under [dual-SDK, dual-device qualification](https://github.com/JiangNanGenius/floe-agent/actions/runs/34846659384) from `a266c0a`. It removes rectangular popover chrome and avoids a retained modal host after repeated selection on iPhone. Production component and UI-test host compilation passed locally; cloud UI qualification and distribution remain pending. Build 156 remains the latest verified TestFlight delivery.
