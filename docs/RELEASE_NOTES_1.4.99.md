## Floe Agent 1.4.99 (Build 130)

### 简体中文

- 修复稳定 SDK 下 PDF 区域编辑后的中文复制/搜索字符漂移：保留经过验证的原生文件字节，避免无修改的 PDFKit 二次重写，并对最终保存文件逐页验证文本。1.4.98（129）候选被发布测试拦截，未上传 TestFlight；本版包含该候选的全部更新。
...
- Fixed CJK copy/search character drift after native PDF region editing on the stable SDK. Preserve verified native bytes instead of redundant PDFKit serialization, and verify each saved page's text. The 1.4.98 (129) candidate was stopped by release tests before TestFlight upload; this release includes all of its changes.
