# Floe Agent 1.4.26 (Build 57) TestFlight Notes
...
- **Fonts:** In **Data Management → Font Resources**, import a TTF/OTF from Files and add another from a direct public HTTPS URL. Confirm duplicates are reused, the font remains available after relaunch and can be used from a different workspace. Ask the Agent to handle a missing document font: `font.list`, bounded `font.install` and `font.resolve` should not wait for approval-model review; `font.remove` must remain reviewed. Private/LAN URLs, credential URLs, non-font files and files larger than 32 MB must fail clearly.
