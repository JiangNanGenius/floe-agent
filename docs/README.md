...
当前为 [191 加急内部交付](qualification/build191-release/README.md)：原上传因发布 SDK 新增 iPhone 助手界面失败而在签名前停止；用户现已同意跳过全部三项失败，修正后的恢复任务35347141494进入签名上传；Apple 可安装状态仍待确认。[190 最终结果](qualification/build190-release/final-results.json)与[封面截图](qualification/build190-release/accepted-ipad-covers-after-relaunch.png)均已保留。
