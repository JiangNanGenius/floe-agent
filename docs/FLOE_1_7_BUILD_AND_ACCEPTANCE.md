...
Build 225 is the active release candidate. It is a source candidate: all four shipping targets declare build 225 and the regenerated project matches. Local evidence covers 23 focused Linux image tests (11 mirror contract, 6 shard staging, 4 download coalescing, 2 install state) plus a Swift 6 iOS-SDK object compile of the real repaired `LinuxGuestImageDownloader.swift` against the typed-throws contract (the same check reproduces the Build 224 line-101 error on the tagged old file), metadata consistency and JSON validation. Build 225 keeps the Build 224 candidate slice (Build 223 Runtime v2 startup/migration repair, local-model retained-task idling, the PPT edit-entry first render, GitHub-primary/Gitee-sharded fallback and the one-way GitHub→Gitee mirror) and repairs the single typed-throws compile error that stopped Build 224's accepted-SDK App build, without weakening fail-closed source classification or cancellation. Build 224 never compiled: immutable tag `v1.7.0-beta.81` / source `c36b7b24` / failed run [35767875337](https://github.com/JiangNanGenius/floe-agent/actions/runs/35767875337) (exit 65, no IPA or upload) are retained as the failure record. Cloud App build, saved IPA, signed upload, Apple processing, Floe QA availability, TestFlight availability, release publication and real-iPad Linux/PPT/MLX/PiP behavior must be reported separately; no release was dispatched for this candidate. See [Build 225 notes](RELEASE_NOTES_1.7.0_BUILD_225.md).
