...
## Preparing: 1.7.0 (225) — source candidate, not yet delivered

All four shipping targets declare build 225 in `FloeAgent/project.yml` and the regenerated `FloeAgent.xcodeproj` matches (eight `CURRENT_PROJECT_VERSION` entries across Debug/Release). Build 225 carries the Build 224 candidate slice (Build 223 Runtime v2 startup/migration repair, MLX residency across retained-task idle windows, the PPT/PPTX edit-entry first render with host framework re-pinned by CI run 35747909238, GitHub-primary/Gitee-sharded Linux image fallback, and the one-way GitHub→Gitee `gitee-mirror` workflow) and repairs the single Swift 6 typed-throws error in `FloeApp/Execution/LinuxGuestImageDownloader.swift` that stopped Build 224's accepted-SDK App build: the `session.bytes` catch is now exhaustive (non-`URLError` failures fail closed as `.responseInvalid`), and `URLError.cancelled` is `.cancelled` only when the task is actually cancelled. Local evidence: 23 focused Linux image tests pass (11 mirror contract, 6 shard staging, 4 download coalescing, 2 install state) under the Xcode 27.0 toolchain, and the real repaired App source compiles to object code under Swift 6 with the iOS 26.0 Simulator SDK against a typed-throws contract stub; the same check reproduces the original line-101 error on the Build 224 file. No cloud build, IPA, upload, Apple processing, Floe QA availability, TestFlight availability, release publication or device acceptance is claimed in this section. No tag was created and no workflow dispatched in this preparation; the planned tag is `v1.7.0-beta.82`. See [release notes](RELEASE_NOTES_1.7.0_BUILD_225.md) and [test notes](TESTFLIGHT_1.7_WHATS_NEW_BUILD_225.json).
