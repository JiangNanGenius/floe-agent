...
- 为云端 AI 功能提供审核期间可用的专用访问方式。不能要求审核员自己购买 API 服务，也不能把开发者主密钥放进应用、公开仓库或截图。提供方、模型名和配置步骤可以进入审核备注；秘密只填入适当的审核私密字段。
...
3. 补齐 Beta 介绍、反馈邮箱、What to Test、审核联系人和可用的演示访问；提交 TestFlight App Review。
