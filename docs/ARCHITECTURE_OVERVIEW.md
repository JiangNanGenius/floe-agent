...
The Linux boundary has two storage models that must not be conflated. Host-side layers (session → project → shared → base, resolved by `EnvironmentRegistry`) provide read-only inheritance and an explicit writable layer per container. Inside Runtime v2, one verified base image is content-addressed and shared; an immutable software template pins a version/digest, and each environment keeps a private delta bound to its boot base, so a pinned environment's changes are captured only against its template while unpinned environments keep base-image behavior. Installed state is therefore private per environment, a saved template never absorbs an environment's installs, and the host's rebuildable cache directories are not installed state. Exactly one writable lease exists per environment; commands, terminals and services in that environment share one guest (files, packages, network, localhost), while different environments get different guests. Pool admission is three-axis (vCPU/RAM/pressure) and SMP-capable boot sits behind two gates: verified image-manifest proof and the frozen per-release shape policy (`GuestReleaseShapePolicy.production`, exactly one hart), which engine capacity, the image manifest and the device quota can never widen — dual-hart requests therefore fail closed with an actionable reason and the performance tier is not enabled. The IDE Run sheet now exposes honest automatic/one-core/two-core choices and states the automatic plan's effective shape; an explicit two-core request is refused under this release (never silently downgraded), and that source change has no App build or device retest yet. Installation state and the VM runtime identity are separate: a stopped environment keeps its disk, and the next start is a new runtime with a new identity.
