...
The first public-beta Creative Mode surface is a native infinite-canvas MVP, not a replacement for the existing Task/Run/Workspace model. Floe remains chat-first: ordinary Chat continues through the existing hierarchy. Creative Mode requires an enabled default image-generation model, while video is an optional extra. A private canvas can be opened without creating a Workspace; optional Workspace canvases remain available for project-owned material and explicit file export. Each canvas contains multiple user-facing documents with movable text, freehand strokes, pan and zoom. It also provides a bounded Canvas Agent research sheet backed by the production Conversation/Run/checkpoint path: only `web.search`, `web.fetch`, and explicitly canvas-authorized MCP tools enter its exact tool ceiling, and the user must explicitly insert a result into the canvas. The broader artifact library and project-context projection described below remain staged expansion work and are not implied by the MVP.
