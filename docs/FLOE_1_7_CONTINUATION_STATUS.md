...
- 手记独立助手复用现有任务与权限服务；普通对话和画布助手可显式选择手记，默认只读。选择器显示并撤销授权，编辑提交时再次检查授权；撤销不删除已发送的对话副本。
...
1. 手记完整 UI、页面图文操作、回答保存与分页、Office 恢复入口均已继续接线，仍需完整交互与保存重开验收；新任务首页资料选择、历史来源定位和导图图片等剩余入口仍待完成。
