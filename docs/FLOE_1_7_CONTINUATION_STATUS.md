...
当前准备候选为 **1.7.0（151）/ `v1.7.0-beta.8`**。此前 149 被执行测试拦下；150 的执行卡顿已修复，但本地化键名检查失败，均未上传 Apple。定向 150 执行 + 83 安全 + 12 工具权限测试通过，双端原生组件最新通过证据为 `4550b6d`。下文保留历史失败与修复过程，不能把早期“待确认”当作最新结论。
