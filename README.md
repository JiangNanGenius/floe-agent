...
**Current internal TestFlight: 1.7.0 (219).** Apple VALID, unexpired and IN_BETA_TESTING in the existing private Floe QA group, verified September 21 at 22:01 UTC. English and Simplified Chinese test notes were saved and read back. Device acceptance belongs to the user. [Build 219 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_219.md) · [Delivery record](docs/TESTFLIGHT_1.7.0_BETA.md).

Build 219 consolidates TinyEMU/Linux as the primary local runtime. The first use of Shell, guest Python, Node, APT/DPKG or a background service automatically downloads, verifies, installs and starts one shared Linux component, then resumes the original command; **Settings → Execution Environments** and the Terminal expose the same state with explicit download, update, start and stop controls, and the guest configures and reports its own network before answering capability queries. Workspace Office previews open the standalone full-screen editor while IDE file-tree documents keep embedded tabs, the IDE refreshes source control after repository changes and foreground return, and ZIP, TAR and 7z archives open in a bounded browser that states when a format needs the Linux runtime instead of starting it silently. Local MLX loading validates the installed snapshot and subtracts a running Linux guest's memory from the device budget. Native Python, Node and Ruby payloads stay out of the App; languages and packages are installed in the Linux guest, and the signed WASI command catalog remains an independent path. [Build 219 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_219.md).
...
| Current status | [Implementation status](docs/FLOE_1_7_IMPLEMENTATION_STATUS.md) · [Build 219 notes](docs/RELEASE_NOTES_1.7.0_BUILD_219.md) | [实施状态](docs/FLOE_1_7_IMPLEMENTATION_STATUS.md) · [Build 219 说明](docs/RELEASE_NOTES_1.7.0_BUILD_219.md) |
