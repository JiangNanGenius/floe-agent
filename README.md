...
Floe 1.7 upgrades the iPad-first Notes workspace, Office, dynamic maps, unified speech, image/video workbenches and project/session environments. **Build 191 is the current confirmed internal Floe QA TestFlight release** (recorded status VALID / IN_BETA_TESTING at 2026-09-18 13:43 UTC); build 178 was the previous confirmed release. **Build 195 is the frozen next candidate: the release workflow creates the reserved tag `v1.7.0-beta.52` at the frozen commit, builds once with the accepted upload SDK, retains the unsigned device IPA and private symbols before signing, and uploads it to internal TestFlight; Apple processing and Floe QA visibility are verified and recorded separately. Builds 192 (`v1.7.0-beta.49`) and 193 (`v1.7.0-beta.50`) stopped in their accepted-SDK App compiles and were never uploaded; build 194 (`v1.7.0-beta.51`) built and uploaded successfully but its publish step hit a release-workflow defect, so it was never published. All three immutable tags remain as evidence.** Production release remains separate. See the [TestFlight record](docs/TESTFLIGHT_1.7.0_BETA.md).
...
Floe Agent **1.7.0 (build 191)** is the latest build recorded as available to the **Floe QA internal TestFlight group**. See the [TestFlight availability record](docs/TESTFLIGHT_1.7.0_BETA.md) and the [build 191 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_191.md). Build 195 is the frozen next candidate: the release workflow creates `v1.7.0-beta.52` at that commit and uploads it to internal TestFlight, with Apple processing and Floe QA visibility verified separately before it is called installable ([Build 195 notes](docs/RELEASE_NOTES_1.7.0_BUILD_195.md)). Builds 192 and 193 never uploaded (compile failures) and build 194 uploaded but was never published (release-workflow defect) — [192](docs/RELEASE_NOTES_1.7.0_BUILD_192.md), [193](docs/RELEASE_NOTES_1.7.0_BUILD_193.md), [194](docs/RELEASE_NOTES_1.7.0_BUILD_194.md) records. Earlier 1.5.3 evidence is retained in the [historical verification record](docs/RELEASE_VERIFICATION_1.5.3.md).
