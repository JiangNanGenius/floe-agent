...
**Build 224 is the current release candidate.** It repairs the Build 223 Runtime v2 startup/migration chain (fresh registries and older Linux installs open again, verified content is never mistaken for uninstalled, and environments needing repair fail closed), keeps the on-device MLX model resident through retained-task idle gaps, and repairs the PPT/PPTX edit entry so editing reaches its first editable render. Linux image downloads stay GitHub-first and use the sharded Gitee mirror only after a bounded primary failure, while a one-way GitHub→Gitee workflow mirrors the repository. Source-level tests have passed; cloud build, upload, Apple processing, Floe QA availability and device acceptance are still separate gates. [Candidate notes](docs/RELEASE_NOTES_1.7.0_BUILD_224.md).
