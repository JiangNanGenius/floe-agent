...
**Current internal TestFlight: 1.7.0 (227).** Immutable tag `v1.7.0-beta.84` binds source `9c756864`; [release run 35957256008](https://github.com/JiangNanGenius/floe-agent/actions/runs/35957256008) compiled with Xcode 26.6, retained the unsigned IPA (739,368,613 bytes; SHA-256 `a77b3b9a120a55dd6737bf1fb89efe7609c8917ca3facab7cd9cbb5c4c66b30c`) before signing, accepted the signed upload and published the [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.84). Apple build `640e39a2-001b-4672-9b17-b4a378d9eb6a` was verified `VALID`, unexpired and `IN_BETA_TESTING` in the sole private internal Floe QA group at 2026-09-24 05:42 UTC by [verify run 35961062720](https://github.com/JiangNanGenius/floe-agent/actions/runs/35961062720), and both test-note languages were read back. [Build 227 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_227.md) · [Delivery record](docs/TESTFLIGHT_1.7.0_BETA.md).

**Build 227 is the current internal delivery.** It carries the feature set tracked in the [Build 226 candidate notes](docs/RELEASE_NOTES_1.7.0_BUILD_226.md) — the native iPad IDE workbench, compressed-archive operations, reusable Linux template plumbing with private disks (the published template images still need this build's install-path checks before either is described as user-downloadable), Runtime v2 resource management, background/PiP and notification repairs, local-model tool continuation, a multiline composer with per-conversation drafts, one third-party license entry, native-first media routing and the pinned Office host. Build 226 itself never produced an IPA: its cloud build stopped because the file-tree compression entry lacked a `FloeTools` import, and Build 227 fixed that from a new immutable source. Cloud build, retained unsigned IPA, signed upload, Apple processing and test-group availability each have separate evidence and do not establish physical-device behavior.

**Known physical-device regressions in Build 227 (not accepted).** iPad testing of this build on 2026-09-24 reported: a downloaded MLX local model crashes in ordinary chat and in the benchmark, independent of tools; a PPT/PPTX preview opens but the edit entry then stalls; and Word/Excel/PPT documents opened from the IDE file tree remain on the opening indicator. Narrow-width Git sidebar layout also needs work. These are tracked as open regressions for the next candidate — not fixed, waived or covered by any pass. PPT editing, local-model load and benchmark, PiP, notifications, keyboard/IME input and dual-core guest operation all remain unaccepted on a physical device.
...
**Source status — unreleased repairs after Build 227 (no new build announced).** `main` is ahead of the delivered Build 227 with work in progress that is not part of any delivered build: a bounded extent-bootstrap fallback for the PPT edit entry, IDE Office tabs resolved through the shared document session, touch-sized IDE Git sidebar actions at narrow widths, and Linux guest run-shape/core-choice plumbing. Component checks do not replace the cloud App build, TestFlight availability and physical-device acceptance, and none of the Build 227 regressions above is fixed until a later build passes those gates.
