...
The next internal candidate, build 179 / beta.36, is in preparation and **not uploaded**: Notes library covers render Word/Excel/PPT and import DXF/DWG with a read-only CAD preview; Office gains host-owned embedded controls, explicit save and per-document annotate settings; the IDE runs the current file with save-before-run and remote-language routing; Shell installs signed Lua via `apt install floe/lua`; local MLX models disable compiled traces process-wide. The reported iPad local-model chat crash is not confirmed fixed, the PHP browser prototype is not included, and cloud CI run 35202845926 failed at IDE native saves and is under investigation. See the [candidate notes](docs/RELEASE_1.7.0_BETA_36.md) and the [build 178 feedback repair record](docs/FLOE_BUILD178_FEEDBACK_REPAIR.md); build 178 remains the latest available TestFlight build.
