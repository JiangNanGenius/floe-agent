...
**Build 220 is the current upload candidate.** It repairs Linux persistence and install state (per-environment grow-only ext4 disks, writable `/floe/env` caches, coalesced installs and a truthful installed/running state), corrects the 9P semantics behind `ls -l` error 524, adds an explicit **keep running in the background** switch with measured-only metrics and honest interruption reporting, unifies task-completion notifications into one foreground in-app banner with authorized background alerts and deep links, and gates PPTX editing on the engine's real edit entry plus a visible render. This line describes source status only; Build 220 is not installable until the cloud build, upload, Apple processing and Floe QA availability are verified. [Candidate notes](docs/RELEASE_NOTES_1.7.0_BUILD_220.md).
