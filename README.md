...
Floe 1.7 upgrades the iPad-first Notes workspace, Office, dynamic maps, unified speech, image/video workbenches and project/session environments. **Build 178 remains the last confirmed internal Floe QA TestFlight release** (recorded status VALID / IN_BETA_TESTING); Apple discovery on 2026-09-18 still lists178 as the latest VALID upload; production release remains separate. See the [TestFlight record](docs/TESTFLIGHT_1.7.0_BETA.md).
...
**Build191 (`v1.7.0-beta.48`, source `715cbc42`) is in expedited internal TestFlight distribution after the user waived all three recorded UI failures.** [The first reuse upload](https://github.com/JiangNanGenius/floe-agent/actions/runs/35343019320) stopped before signing after a third UI failure appeared in the accepted-SDK iPhone assistant readiness test. The user also waived this third failure. [The corrected recovery upload](https://github.com/JiangNanGenius/floe-agent/actions/runs/35347141494) reuses the preserved device artifact without an App rebuild. Both SDK App regression suites passed204/204 and NativeNotes passed101/101 on each simulator family. Two SDK27 UI failures remain recorded and were explicitly waived for this internal device-testing delivery; this is not full acceptance. Signing, Apple processing and Floe QA installability are still pending. [Current evidence](docs/qualification/build191-release/README.md), [retained cover screenshot](docs/qualification/build190-release/accepted-ipad-covers-after-relaunch.png). Office covers can be labelled content summaries, not exact native page renders. Physical local-model and Office/Pencil acceptance remains with the tester.
