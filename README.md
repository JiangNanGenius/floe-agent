...
**Build 223 is the current release candidate.** It introduces TinyEMU Runtime v2 with one content-addressed base image, per-environment CoW state, a four-VM pool and a 1.5–2 GiB dynamic budget; it also repairs legacy-image/9P migration, Linux/MLX arbitration, local-model multi-turn tools and bounded PPT opening. Source compilation and focused policy tests have passed; cloud build, upload, Apple processing, Floe QA availability and device acceptance are still separate gates. [Candidate notes](docs/RELEASE_NOTES_1.7.0_BUILD_223.md).
