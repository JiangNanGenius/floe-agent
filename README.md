...
**Build191 (`v1.7.0-beta.48`, source `715cbc42`) is proceeding to expedited internal TestFlight at the user's request.** [The existing device artifact is reused](https://github.com/JiangNanGenius/floe-agent/actions/runs/35343019320), without another App build. Both SDK App regression suites passed204/204 and NativeNotes passed101/101 on each simulator family. Two SDK27 UI failures remain recorded and were explicitly waived for this internal device-testing delivery; this is not full acceptance. Signing, Apple processing and Floe QA installability are still pending. [Current evidence](docs/qualification/build191-release/README.md), [retained cover screenshot](docs/qualification/build190-release/accepted-ipad-covers-after-relaunch.png). Office covers can be labelled content summaries, not exact native page renders. Physical local-model and Office/Pencil acceptance remains with the tester.
