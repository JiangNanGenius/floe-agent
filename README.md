...
**Current internal TestFlight: 1.7.0 (229).** Immutable tag `v1.7.0-beta.86` binds source `b06b0b0e`; [release run 36239956371](https://github.com/JiangNanGenius/floe-agent/actions/runs/36239956371) compiled with Xcode 26.6, retained the unsigned IPA (739,485,403 bytes; SHA-256 `5cd022eb612d89f1d94b91594b747000404cec3f247b8508e89a83e88436124d`) before signing, uploaded the signed build and published the [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.86). [Prepare run 36242606863](https://github.com/JiangNanGenius/floe-agent/actions/runs/36242606863) read back both beta-note languages; [verify run 36242653374](https://github.com/JiangNanGenius/floe-agent/actions/runs/36242653374) confirmed Apple `VALID`, unexpired and `IN_BETA_TESTING` in the sole private internal Floe QA group. [Build 229 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_229.md) · [Delivery record](docs/TESTFLIGHT_1.7.0_BETA.md).

**Build 229 targets the latest iPad reports.** Office close/save acknowledgements now settle once, a late real PPT paint can recover a waiting session, and a failed original-file write-back keeps its working copy. The local-model lifecycle evicts a failed or unclean MLX container before the next turn and respects benchmark leases. Two-core Linux remains unavailable: the launch path exists, but the shipping gate stays at one vCPU until correct, faster guest execution is qualified. Build and upload evidence does not establish physical-device behavior.

**Physical-device acceptance remains open.** The iPad still needs to confirm PPT open/edit/save/reopen in Workspace, Notes and IDE; Office exit and cancellation; and local-model loading, benchmark, multi-turn chat and tool use. The packaged Office host was verified in the IPA, but its cloud receipt did not open an engine or prove a painted slide. PiP, notifications and keyboard/touch behavior also remain device checks.
...
**Distribution channels are separate.** GitHub has the Build 229 prerelease and unsigned IPA; Apple validation and Floe QA group verification have passed. [Gitee refs run 36243056531](https://github.com/JiangNanGenius/floe-agent/actions/runs/36243056531) verified the source mirror, and [release-sync run 36242058529](https://github.com/JiangNanGenius/floe-agent/actions/runs/36242058529) created the matching [Gitee prerelease](https://gitee.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.86) with six hash-verified small assets. The 705.2 MiB IPA could not be attached because the 1 GiB repository attachment quota had only 28.9 MiB free; use the GitHub IPA.
