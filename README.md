...
**Current internal TestFlight: 1.7.0 (221).** Apple VALID, unexpired and IN_BETA_TESTING in the sole private internal Floe QA group, verified September 22 at 03:13 UTC. English and Simplified Chinese test notes were saved and read back. Device acceptance belongs to the user. [Build 221 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_221.md) · [Delivery record](docs/TESTFLIGHT_1.7.0_BETA.md) · [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.78).

**Build 222 is the current release candidate.** It introduces TinyEMU Runtime v2 with one content-addressed base image, per-environment CoW state, a four-VM pool and a 1.5–2 GiB dynamic budget; it also repairs legacy-image/9P migration, Linux/MLX arbitration, local-model multi-turn tools and bounded PPT opening. Source compilation and focused policy tests have passed; cloud build, upload, Apple processing, Floe QA availability and device acceptance are still separate gates. [Candidate notes](docs/RELEASE_NOTES_1.7.0_BUILD_222.md).
