...
**Current internal TestFlight: 1.7.0 (228).** Immutable tag `v1.7.0-beta.85` binds source `ed8f233a`; [release run 36009125622](https://github.com/JiangNanGenius/floe-agent/actions/runs/36009125622) compiled with Xcode 26.6, retained the unsigned IPA (739,478,368 bytes; SHA-256 `1cfe17ba4e95f869bf962d3a1238f333b6525fddfc1dde1b295f78a262d95076`) before signing, uploaded the signed build and published the [GitHub prerelease](https://github.com/JiangNanGenius/floe-agent/releases/tag/v1.7.0-beta.85). [Prepare run 36015637207](https://github.com/JiangNanGenius/floe-agent/actions/runs/36015637207) read back both beta-note languages; [verify run 36015717443](https://github.com/JiangNanGenius/floe-agent/actions/runs/36015717443) confirmed Apple `VALID`, unexpired and `IN_BETA_TESTING` in the sole private internal Floe QA group. [Build 228 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_228.md) · [Delivery record](docs/TESTFLIGHT_1.7.0_BETA.md).

**Build 228 targets the reported Build 227 regressions.** It changes IDE Office-tab opening, the PPT editor host's bounded entry path, narrow Git controls and Qwen MLX prefill, and moves Linux conflict confirmation to local-model selection. The IDE Run sheet now offers automatic, one-core and two-core requests. Two-core execution is still disabled in the shipping App: corrected cloud guest tests passed correctness checks but missed the measured performance threshold. Build and upload evidence does not establish physical-device behavior.

**Physical-device acceptance remains open.** Build 227 iPad testing reported ordinary MLX chat/benchmark crashes, PPT edit-entry stalls and indefinite IDE Office loading. Build 228 contains source repairs and passed targeted cloud component checks, including a macOS Qwen prefill host run, but has not yet passed those flows on an iPad. PPT edit/save/reopen, local-model chat and benchmark, PiP, notifications and keyboard/touch behavior remain to be checked on the device.
...
**Distribution channels are separate.** GitHub has the Build 228 prerelease and unsigned IPA. Gitee has the matching source/tag and small assets; its 1 GiB repository attachment quota has only 28.9 MiB free, so the 705.2 MiB IPA could not be mirrored there. Apple validation and Floe QA group verification for this build have both passed.
