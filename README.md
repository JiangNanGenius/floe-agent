...
The next internal candidate is **build 186 / beta.43**, frozen at source `d421fea2` with tag `v1.7.0-beta.43`; [release run 35306551280](https://github.com/JiangNanGenius/floe-agent/actions/runs/35306551280) is in progress with the Notes component and both SDK App gates in parallel, and upload requires all three. It adds durable GitHub cloud-build recovery and improves Word, Excel and PowerPoint previews in Notes. Follow-up component run 35304310882 passed OCR on both devices but failed iPhone 83/84 on one direct staged Word Quick Look timeout while the same file passed through the real cover service; the six sample tests now use that real importer/CAS/cover path and first execute in this release run. Build 178 remains the last confirmed internal release; nothing is uploaded or installable. Physical iPad local-model chat stability and native Office/Pencil acceptance are still pending. See the [beta.43 candidate record](docs/RELEASE_1.7.0_BETA_43.md).
