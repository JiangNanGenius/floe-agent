...
Floe 1.7 upgrades the iPad-first Notes workspace, Office, dynamic maps, unified speech, image/video workbenches and project/session environments. **Build 191 is the current confirmed internal Floe QA TestFlight release** (recorded status VALID / IN_BETA_TESTING at 2026-09-18 13:43 UTC); build 178 was the previous confirmed release. **Build 192 is the next reserved build (tag `v1.7.0-beta.49` is reserved and does not exist): it is in preparation, not built, not uploaded and not installable.** Production release remains separate. See the [TestFlight record](docs/TESTFLIGHT_1.7.0_BETA.md).
...
**Build 192 is the next reserved internal candidate and is not built or uploaded.** The working tree carries the build 191 feedback repair; [the reserved Build 192 notes](docs/RELEASE_NOTES_1.7.0_BUILD_192.md) separate implemented behavior from host-level validation and open device acceptance. The repair includes truthful Office editability (an unknown permission is never editable, and an imported advanced workbook may fail a strict save instead of being silently rewritten), file-type-routed IDE tabs with one Office working copy, file-preview sharing, Git fast-forward/conflict/staged-discard recovery, interactive-terminal lifetime fixes, explicit npm/pnpm authority, automatic model fallback, durable video-from-chat jobs with one reference image, and real GIF frame handling. Lua 5.4.8 remains the only installable signed language runtime (the catalog's other ready entry, `floe/wasm-text`, is a text utility); Ruby and PHP are compilepending candidates. No upload-SDK App build has run for this source, and the build 191 local-model crash is not proven fixed. This is planning documentation, not a release or availability claim.
...
Floe Agent **1.7.0 (build 191)** is the latest build recorded as available to the **Floe QA internal TestFlight group**. See the [TestFlight availability record](docs/TESTFLIGHT_1.7.0_BETA.md) and the [build 191 release notes](docs/RELEASE_NOTES_1.7.0_BUILD_191.md). Build 192 is the next reserved candidate and is not built or uploaded ([reserved notes](docs/RELEASE_NOTES_1.7.0_BUILD_192.md)). Earlier 1.5.3 evidence is retained in the [historical verification record](docs/RELEASE_VERIFICATION_1.5.3.md).
