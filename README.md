...
Signed builds are distributed through TestFlight when a testing group is available. The current source target is Floe Agent 1.4.77 (build 108); consult [Releases](https://github.com/JiangNanGenius/floe-agent/releases) and TestFlight for builds that actually completed every release gate. A source version or tag alone does not prove that Apple received or processed a build.
