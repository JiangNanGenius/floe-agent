...
新候选版本为 **1.6.3（138）**，正在准备发布；可安装状态以 TestFlight 核验为准。参见[测试版说明](docs/RELEASE_NOTES_1.6.3.md)。本轮以真实会话日志为证据、对标 OpenCode / Claude Code / Kimi Code，硬化代理 harness 本体：

- **任务清单保持干净**：完成的清单自动完结，新任务自动开新清单——不再无限追加；更新只需携带未完成步骤 ID，`expectedRevision` 可选，校验报错精确到具体步骤。
- **失败熔断与计划保鲜提醒**：同一工具连续失败三次即被拦截并强制重读 schema；工作跑在计划前面时运行中提醒。以权威 `<system-reminder>` 注入。
- **模型撰写的上下文压缩**（云端默认开启，确定性回落保留）+ 系统提示词全面重写：交付必须真实验证才可声称完成；遇卡点报告而非静默缩水；被拒绝不再原样重试或绕道。
- **类型化并行子代理**：`delegate` 增加 explore/research 预设与自包含交接契约；同批次委托真正并行。
- **更诚实的悬浮窗（PiP）**：真应用图标、decode 口径的模型速度（工具输出与预填不再计入）、实时工具/审批细节。
- **后台任务提交即校验**：参数名写错在提交当场报出缺失键名，不再异步失败。
- **首批 wheelhouse 转正**：regex（原生双 slice）、PyYAML、MarkupSafe 走完"构建→不可变发布→SHA pin"全链路；zstandard/Brotli/greenlet/frozenlist/multidict 候选在途。
