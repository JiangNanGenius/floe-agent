...
新候选版本为 **1.6.4（140）**，正在准备发布；可安装状态以 TestFlight 核验为准。参见[测试版说明](docs/RELEASE_NOTES_1.6.4.md)。本轮按真机实测反馈快速迭代：

- **兼容模式工具命名全链路**：三通道统一正反向映射，反向映射覆盖整个权限上限集合；tools.list 正确翻页兼容游标；本地回退归一化拼写漂移并纳入发现工具。
- **PDF 闪退面关闭**：PDFKit 触点进程级串行化、渲染单次上限、merge 生命周期修复、大文件阅读解析移出主线程。
- **网络诊断做对**：设备端真实 ICMP ping、BSD/Linux `-W` 语义修正、traceroute 消除逐跳 DNS 停顿、回退有界、dnsLookup 有截止。
- **任务列表实时刷新**：运行进展即刻反映到列表，无需手动刷新。
- **更多内置 Python 包**：zstandard、Brotli、greenlet、frozenlist、multidict 加入 pin 制捆绑运行时。
