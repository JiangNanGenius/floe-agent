...
下一内部候选为 **185 / beta.42，准备云端验收，尚未上传**。包含 IDE 云端构建持久化、重开后主动回读与轮询、Lua 环境变量限制修复，以及 Word／Excel／PPT／CAD 真实内容缩略图。184 的两套 SDK 编译已通过，但同一个 Lua 回归失败阻止上传，原始证据与恢复包已保留。详见[候选记录](docs/RELEASE_1.7.0_BETA_42.md)。178 仍是最后确认可用的内部 TestFlight 版本；真机本地模型普通聊天稳定性另行验收。
