...
下一内部候选为 **Build 182 / beta.39，云端验收失败，尚未上传**：加入 IDE 云端构建的持久任务与自动回读，并修复 Office／CAD 文件内容封面。Build 179 的 SDK 27 安装包已留存，但发布 SDK 的手记步骤超时，未进入上传；此前 IDE 保存已单独通过双端模拟器验收，不能据此宣称新候选已验收。详见[候选说明](docs/RELEASE_1.7.0_BETA_39.md)、[缩略图验收](docs/NOTES_OFFICE_THUMBNAIL_ACCEPTANCE.md)与[修复记录](docs/FLOE_BUILD178_FEEDBACK_REPAIR.md)。Build 178 仍是最后确认的内部 TestFlight 版本；iPad 本地模型普通聊天闪退尚未确认根治。
