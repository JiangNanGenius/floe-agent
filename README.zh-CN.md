...
- **在工作区内展开创意。** 每个工作区可打开一个原生无限画布项目，在其中管理多张画布、移动文本、自由手绘、平移和缩放。
...
- **创建并修改 Office 文件。** 可在本机生成 DOCX、XLSX 和 PPTX，先预览，再进入 Floe 的基础原生编辑器手工修改文字、单元格/公式、幻灯片和演讲者备注，文档不会因此上传到云端。
...
项目会在测试组开放时通过 TestFlight 分发签名版本。当前源码目标版本为 Floe Agent 1.4.55（build 86）；只有同时通过发布门禁并能在 TestFlight 中看到的构建才算完成发布。仅有源码版本或标签不能证明 Apple 已收到或处理该构建。
...
| 工作区画布 | 在原生无限平面中管理多张画布、文本、适配 Pencil 的手绘、平移和缩放。 |
