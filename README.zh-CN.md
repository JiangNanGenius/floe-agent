...
Floe 1.7 面向 iPad 优先升级手记、Office、动态导图、统一语音、图像与视频工作台，以及项目/会话运行环境。**191 是最后确认的内部 TestFlight 交付版本**（2026-09-18 13:43 UTC 核实 VALID／IN_BETA_TESTING）。**Build195 为已冻结的下一候选：发布工作流在冻结提交创建预留标签 `v1.7.0-beta.52`，用验收上传 SDK 只构建一次，签名前留存未签名设备 IPA 与私有符号，然后上传内部 TestFlight；Apple 处理与 Floe QA 可见性单独核验并记录。Build192（`v1.7.0-beta.49`）与 Build193（`v1.7.0-beta.50`）在验收上传 SDK 编译阶段失败、未上传；Build194（`v1.7.0-beta.51`）已完成构建并成功上传，但其发布步骤命中发布工作流缺陷，未能发布。三个不可变标签均作为证据保留。**正式发布另行安排。分发状态见[TestFlight 记录](docs/TESTFLIGHT_1.7.0_BETA.md)。
