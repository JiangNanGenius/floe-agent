...
新候选版本为 **1.6.2（137）**，正在准备发布；可安装状态以 TestFlight 核验为准。参见[测试版说明](docs/RELEASE_NOTES_1.6.2.md)。本轮强化工具链本身与中文文档体验：

- **后台任务（jobs.\*）**：大文件下载与 Python 数据抓取/清洗不再阻塞当前任务——立即返回 jobID，对话继续，完成后结果自动回到对话；下载在应用挂起后继续并透明断点续传（上限 2 GB）。
- **工具结果随对话回放**：模型读过的证据跨轮保留，不再反复枚举工具目录；schema 预算卸载会明确提示，已加载工具定义跨任务保留。
- **精选中文字体包**：9 族核心字体（思源黑体/宋体简繁、霞鹜文楷、HarmonyOS Sans、MiSans、阿里巴巴普惠体 3.0、更纱黑体 Mono）精选字重让 Office 预览/编辑正确渲染中文；另有 19 族展示字体可按需自助安装。
- **Office 正确性**：提取/编辑不再吞并自闭合 XML 元素（空白样式单元格、空段落）；引擎启动后空闲预热。
- **Harness 成本控制**：提示词静态/动态分层并提供 Anthropic 显式缓存断点；100 步工具迭代软预算强制失控循环收尾。
