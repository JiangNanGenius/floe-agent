...
description: Device DNS, TCP and ICMP ping diagnostics, host ICMP and traceroute, LAN discovery, HTTP and downloads.
---
...
- `network.ping` runs a genuine bounded ICMP echo on the device by default (datagram ICMP socket, 1-10 probes at a 1s interval, per-reply RTT with min/avg/max and loss summary). `network.traceroute` remains host-only: device execution returns deviceTracerouteUnavailable. Do not retry unchanged calls or report a simulated success. For a genuine remote probe, select executionTarget=host with a paired hostID only when the user wants that host as the source. A host probe does not measure the phone's network path.
