...
+ (NSDictionary<NSString *,id> *)runScript:(NSString *)script
    ...
    static const char *runner =
        ...
        "class _FloeSink(_io.TextIOBase):\n"
        ...
        "_floe_loggers=[_floe_logging.root]+[v for v in _floe_logging.Logger.manager.loggerDict.values() if isinstance(v,_floe_logging.Logger)]\n"
        "_floe_logging_state={v:(list(v.handlers),list(v.filters),v.level,v.disabled,v.propagate) for v in _floe_loggers}\n"
        "_floe_logging_disable=_floe_logging.Logger.manager.disable\n"
        "_floe_old_handlers={h for v in _floe_loggers for h in v.handlers}\n"
        "try:\n"
        ...
        " _floe_closed_handlers=set()\n"
        " for _floe_logger in [_floe_logging.root]+[v for v in _floe_logging.Logger.manager.loggerDict.values() if isinstance(v,_floe_logging.Logger)]:\n"
        "  for _floe_handler in list(_floe_logger.handlers):\n"
        "   if _floe_handler not in _floe_old_handlers and _floe_handler not in _floe_closed_handlers:\n"
        "    try: _floe_handler.close()\n"
        "    except Exception: pass\n"
        "    _floe_closed_handlers.add(_floe_handler)\n"
        "  _floe_saved=_floe_logging_state.get(_floe_logger,([],[],0,False,True))\n"
        "  _floe_logger.handlers[:]=_floe_saved[0]; _floe_logger.filters[:]=_floe_saved[1]; _floe_logger.setLevel(_floe_saved[2]); _floe_logger.disabled=_floe_saved[3]; _floe_logger.propagate=_floe_saved[4]\n"
        " _floe_logging.disable(_floe_logging_disable)\n"
