...
`shell-results.json` must contain eleven records in order [2,3,1,0,4,5,6,7,8,9,10]:
...
Cases 9 and 10 reject a missing literal pipeline consumer with exit 127 and then print `after_missing_pipe`; both must release the worker.
