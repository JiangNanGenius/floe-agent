...
1. Validate the exact simulator App artifact and Xcode's embedded simulated Keychain identity. CI builds it with normal simulator signing enabled; post-build ad-hoc signing cannot substitute for Xcode's simulated entitlement section. The recording workflow never modifies the App artifact. Install it and let normal migrations finish.
