...
@12 echo floe-9p-proof > /mnt/floe_smoke_9p.txt && echo FLOE_SMOKE_9P_OK
@14 ifconfig eth0 10.0.2.15 netmask 255.255.255.0 up
@16 ping -c 1 -W 3 10.0.2.2 && echo FLOE_SMOKE_PING_OK
@20 echo FLOE_SMOKE_DONE
EOF
...
echo "== evidence =="
grep -a "FLOE_SMOKE" "$WORK/smoke-transcript.txt" || true
if [ -f "$WORK/share9p/floe_smoke_9p.txt" ]; then
  echo "9p host-share file present: $(cat "$WORK/share9p/floe_smoke_9p.txt")"
else
  echo "9p host-share file MISSING"; rc=3
fi
echo "smoke exit: $rc (0 = all markers observed)"
exit $rc
