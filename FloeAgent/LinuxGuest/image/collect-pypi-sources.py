...
def build_pypi_bundle(recipe_path, out_dir, github_run=""):
    ...
    for distribution in sorted(pypi):
        ...
        # Only two small text files are needed from the sdist. Never unpack
        # the archive in the privileged source-collection job: a tar symlink
        # can redirect later members even if every name looked safe before
        # extraction.
        sdist_dir = Path(tempfile.mkdtemp(prefix="pypi-sdist-tree-", dir=str(out)))
        try:
            sdist_root_name = sdist_dest.name[:-len(".tar.gz")]
            with tarfile.open(str(sdist_dest), "r:gz") as archive:
                def read_source_member(relative_name):
                    name = sdist_root_name + "/" + relative_name
                    matches = [member for member in archive.getmembers()
                               if member.name == name]
                    if len(matches) != 1 or not matches[0].isfile() or matches[0].size > 65536:
                        raise RuntimeError("missing, ambiguous or oversized sdist member: " + name)
                    stream = archive.extractfile(matches[0])
                    if stream is None:
                        raise RuntimeError("unreadable sdist member: " + name)
                    return stream.read(65537).decode("utf-8")

                base_py = read_source_member("setupsrc/base.py")
                pyproject = read_source_member("pyproject.toml")
            pin_match = re.search(r"^SBUILD_NATIVE_PIN\s*=\s*(\d+)\s*$",
                                  base_py, flags=re.MULTILINE)
