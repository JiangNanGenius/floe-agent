...
| `run_feedback_shell_bridge_host.sh` | The real `FloeShellBridge.mm` compiled for macOS against a scripted stub engine: gate Busy vs TimedOut, cooperative cancellation releasing the gate via the worker's own teardown (next command runs immediately), quarantine of a non-cooperative timed-out/cancelled worker (next command reports Busy until the old worker is proven stopped), bounded finalization with a descendant holding the output pipe open, large-output caps, repeated commands, readiness drain, descriptor claim/close/end ownership, repeated interactive exchange, and an already-cancelled session open on a free gate (Cancelled, no engine entry, no session record, gate still exactly available). | Xcode clang++; no iOS SDK needed. |
| `run_feedback_shell_session_io_host.sh` | The real `SessionIO` pump spliced from `IOSSystemShellBackend.swift` and compiled with `swiftc` against stub seam types: `sendEOF()` flushes every pending byte first and then closes stdin (queued input then a real EOF), input after EOF is rejected, a hard EPIPE closes the owned stdin descriptor exactly once while output drains continue, and a queue larger than the pipe buffer survives EAGAIN/partial-write retries completely and in order. Extraction also asserts the single-close/EOF-request source invariants. | Xcode `swiftc`; no iOS SDK needed. |
...
| `test_dash_framework_provenance.py` | `Frameworks/dash-build-manifest.json` (written by `scripts/build_dash_ios.sh`) matches the tracked DashIOS build inputs and the built `dash*.xcframework` binaries. Inputs are the non-documentation DashIOS files, the build script and the FloeShellEngine artifact pin; documentation (PROVENANCE.md, COPYING, man pages) is ignored, while compiled-source, build-configuration, linked-engine-pin and binary drift fail closed. Release CI runs this in check mode after bootstrapping; `--self-test` exercises the classification in a temporary tree. | `python3`. |
| `test_feedback_python_runner.py` | Retired with the bundled CPython bridge in Phase 2 (TinyEMU migration); archived at `FloeAgent/ThirdParty/NativeRuntimeArchive/tests/`. | archived |
