// Build191 rich presentation + DOCX roundtrip structure checks (local runner
// port of FloeAgent/Tests/FloeDocumentsTests/RichDeckChecksTests.swift, which
// stays the canonical suite for the cloud package build).
//
// This standalone `@main` variant exists so the same checks can be compiled
// directly against the current FloeDocuments sources with swiftc - no SwiftPM,
// no App build - and produce a real deck for the independent python-pptx
// read-back (`validate_rich_pptx.py`). Structure-level checks only: they do not
// prove native engine or real Office UI behaviour. The deck must embed
// ppt/embeddings/floe-chart-data-<n>.xlsx (the landed chart-workbook marker).
//
// Keep this file in sync with the tracked Swift Testing test body.
import Foundation
...
@main
struct RichDeckHarness {
    static func main() async {
        do {
            try await run()
        } catch {
            failed.append("harness error")
            print("FAIL harness error — \(error)")
        }
        print("\nchecks passed=\(passed) failed=\(failed.count)")
        if !failed.isEmpty {
