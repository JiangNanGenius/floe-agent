...
public enum BundledDomainSkills {
    ...
    public static let all: [Definition] = officialDefinitions + [
        ...
        Definition(
            ...
            - Packages: `apt` in this shell is Floe's APT-compatible capability catalog, **not a Debian system package manager**. `apt list` / `search` / `show` are read-only catalog queries; `install` (with `purpose`) only provides reviewed Floe capabilities — signed WASI commands such as `floe/lua`, `floe/ruby`, `floe/php` and `floe/wasm-text`, managed pure-Python packages, fonts, local models and data-only `.deb` payloads. Without an environment that configures Debian sources there is no system package index, so system tools (`bash`/`zsh`, `ps`, `setsid`, `nohup`, `ssh`/`scp`, `zip`/`7z`, `xz`/`bzip2`, the `sqlite3` CLI) cannot be installed; use `workspace.archive`, Python's `zipfile`/`tarfile`/`lzma`/`sqlite3`, `exec.localService` for long-running services, and the `git.*` or `ssh.*` tools for repository and remote work. `git` in the shell only prints that guidance. `pip` / `python3 -m pip` work with the managed Python service. `npm install` / `pnpm add` run through the managed environment service (install location and lifecycle scripts are environment-owned) and refuse an unbound shell or unsupported options with an explicit reason instead of installing into the workspace. `apt-get` and `pkg` share the same environment service as Settings. Do not call the retired apt Agent tool or try to install ordinary Linux ELF payloads.
