...
/// Linux-style apt/dpkg command surface for Debian packages only. The app
/// registers `apt`, `apt-get`, `apt-cache`, `apt-mark`, `dpkg` and `dpkg-deb`
/// as replacement commands that route here. Signed WASM capabilities, Python
/// and Node packages are separate families with their own entries and are
/// never resolved through this CLI.
public struct PackagesCLI: Sendable {
    ...
    private func runApt(_ arguments: [String], cancellation: CancellationToken?) async -> Result {
        if arguments.dropFirst().contains("--help") || arguments.dropFirst().contains("-h") {
            return Result(output: "Floe APT-compatible package manager (Debian packages only)\napt [--yes] update | list [--installed] | search QUERY | show PACKAGE | install PACKAGE[=VERSION] | remove PACKAGE | upgrade\nSources and trust are configured in the environment package settings. Signed WASM capabilities and Python/Node packages are separate families with their own entries and are never Debian packages. Unsupported flags fail before making changes.")
        }
        ...
        case "search":
            ...
            let lines = await engine.search(query, container: context.container)
