...
enum ToolWorkflowGuidance {
    static func contextLines(for toolNames: [String]) -> [String] {
        ...
        if names.contains("vnc.observe") {
            lines.append("VNC workflow is a strict state machine: vnc.status -> vnc.connect -> vnc.observe -> input with post-action evidence -> next input using that fresh evidence. VNC actions return screenshots, OCR elements and screenshotSHA256 directly. Call observe again only when evidence is missing, stale, noNewFrame, or explicitly needed for a delayed transition. A partialSuccess result means input may already have executed: do not replay it. Queued input is not server acknowledgement or task success. Never call observe or an input tool while disconnected. An explicit user-requested prerequisite method or tool order is binding for this run: execute that route before any later VNC step. For example, when the user requested SSH setup first, use ssh.listHosts -> ssh.inspectTarget -> ssh.execute in explicit host mode -> ssh.updateHost, then vnc.connect and only then vnc.observe. SSH is an example recovery route, not a mandatory fallback when the user chose another available resolver. If no route was specified, select an available configuration resolver from the tools and current evidence. Never repeat an unchanged non-retryable VNC call and never ask for a password when an existing secure credential reference or an authorized generated setup credential can be stored without exposing it.")
        }
        if names.contains("canvas.getState") {
            lines.append("Canvas workflow: canvas.getState returns canvasID, documentID, revision, and node IDs. Reuse the latest exact revision and IDs for a patch or generation. Mutation results return the new revision and a delta of changed nodes/connections and removed IDs; apply that delta to known state and continue without another inspection. Refresh only when required context is missing or after a revision conflict. Media status IDs come from the generation result.")
        }
