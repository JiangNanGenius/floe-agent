...
public struct LocalPythonTool: AgentTool {
    ...
    public static let toolDescription =
        "Run Python 3.13 privately on this device for scripts, files, JSON, SQLite, XML, archives, dates, async work and data processing. Floe bundles the compatible iOS standard-library extensions, including asyncio, bisect, heapq, pickle, zoneinfo, uuid, statistics, csv, json, sqlite3, mmap, zipfile, tarfile, gzip, bz2, lzma, shutil, hashlib, hmac, secrets, base64, binascii, ctypes, xml.etree.ElementTree and pyexpat. iOS does not provide desktop shell/account modules such as curses, readline, grp, pwd, spwd, syslog or multiprocessing, so do not request them. numpy (2.5.2) and Pillow (11.0, import as PIL) ARE bundled natively as signed frameworks — import and use them directly; do not claim they are unavailable and do not route them to WebAssembly. A task may request an additional pure-Python package with `packages` or a declarative `pipCommand` such as `pip install marko==2.2.0`; include `packagePurpose` and only the capabilities needed for the user's current request. Floe reviews the purpose before downloading and installs only compatible pure-Python packages. Do not invoke pip, ensurepip, subprocess or shell installers inside `script`. For pandas, scipy, matplotlib or another binary package that is not bundled (upstream has not published a compatible iOS build), use the browser-based Pyodide WebAssembly route: create a workspace HTML result, load Pyodide over public HTTPS, run the package there, and exchange bounded input/results as JSON. Never claim a native package was installed when it ran in WebAssembly. PyStata requires a licensed Stata installation and pyreadstat requires native extensions; use exec.compatEvaluator for bounded R/Stata-compatible statistics or an approved configured remote host for the full runtimes. PDF, image, document, batch-processing and data-analysis tasks are expected uses when they remain within the user's request."
