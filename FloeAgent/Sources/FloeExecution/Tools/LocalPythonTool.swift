...
public struct LocalPythonTool: AgentTool {
    ...
    public static let toolDescription =
        "Execute bundled Python 3.13 in Floe's on-device sandbox. The standard library includes statistics, csv, json and sqlite3 for bounded data work. The model may request pip packages with `packages` or a familiar declarative `pipCommand` such as `pip install marko==2.2.0`; provide `packagePurpose` and the narrow `packageCapabilities` actually required. Before download, Floe's package-review model verifies that the package is necessary for the user's existing task, then Floe resolves it in quarantine and atomically installs only runnable platform-independent pure-Python wheels. Package names are not a permission whitelist. `pipCommand` is parsed rather than shell-run; do not invoke pip, ensurepip, subprocess, or shell installers inside `script`. Native wheels and host executables cannot run in CPython on iOS. For on-device numpy, pandas, scipy, matplotlib or another binary scientific package, create a workspace HTML artifact that loads Pyodide from public HTTPS, run the package in the browser's WebAssembly Python runtime, and exchange bounded inputs/results as JSON with the local workflow; do not report that native pip installed it. PyStata requires a separately installed licensed Stata runtime, while pyreadstat uses native extensions, so neither can be installed in this on-device pure-Python sandbox. Use exec.localNumerical for bounded R/Stata-compatible statistics, the Pyodide/WASM browser path for supported scientific Python, or a classified SSH task on a configured licensed host for full R, Stata, PyStata or native packages. Common PDF, image/SVG, document, batch-processing and data-analysis workflows are legitimate when their capabilities stay within the user's request."
