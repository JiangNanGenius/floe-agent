...
public struct SSHUpdateHostTool: AgentTool {
    ...
    public static let toolDescription = "Edit a saved device and its connection profiles. Use ssh.listHosts first. Device kind is optional descriptive metadata and never overrides configured protocols. You may add direct or SSH-tunnel VNC, Telnet, TCP and BLE serial metadata. Omitted fields are preserved; supplied connection arrays replace that connection group. For a new VNC credential, credentialInput may be the plaintext supplied or generated for the current task, or an existing Floe credential reference. Floe stores the value in Keychain and never returns it. Leave credentialInput and clearPassword omitted to preserve an existing credential."
    public static let parametersJSON = #"{"type":"object","properties":{"hostID":{"type":"string"},"displayName":{"type":"string"},"address":{"type":"string"},"port":{"type":"integer","minimum":1,"maximum":65535},"user":{"type":"string"},"isSSHEnabled":{"type":"boolean"},"deviceKind":{"type":"string","enum":["unspecified","linux","mac","windows","nas","router","switchDevice","appliance","other"]},"isRemoteExecutionEnvironment":{"type":"boolean"},"vncConnections":{"type":"array","items":{"type":"object","properties":{"id":{"type":"string"},"displayName":{"type":"string"},"transport":{"type":"string","enum":["direct","sshTunnel"]},"host":{"type":"string"},"port":{"type":"integer","minimum":1,"maximum":65535},"credentialInput":{"type":"string","description":"Plaintext for this setup or a Floe credential reference"},"password":{"type":"string","description":"Deprecated alias for credentialInput"},"clearPassword":{"type":"boolean"}},"required":["displayName","transport","host","port"],"additionalProperties":false}},"auxiliaryConnections":{"type":"array","items":{"type":"object","properties":{"id":{"type":"string"},"displayName":{"type":"string"},"kind":{"type":"string","enum":["telnet","tcp","bluetoothSerial"]},"host":{"type":"string"},"port":{"type":"integer","minimum":1,"maximum":65535},"bluetoothPeripheralID":{"type":"string"},"bluetoothServiceUUID":{"type":"string"},"bluetoothWriteCharacteristicUUID":{"type":"string"},"bluetoothNotifyCharacteristicUUID":{"type":"string"}},"required":["displayName","kind"],"additionalProperties":false}}},"required":["hostID"],"additionalProperties":false}"#
