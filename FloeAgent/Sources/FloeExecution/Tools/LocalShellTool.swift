...
public struct LocalShellTool: AgentTool {
    ...
    public static let toolDescription =
        "Execute local Unix commands or a multi-line POSIX shell script. Use for file/text processing, batch edits, loops, pipelines, redirections, globs, variables and &&/||; compose commands in one script when appropriate. Includes text/archive/hash commands, python3, node, network diagnostics and installed signed WASI floe-* commands in the current task workspace; `git` is not provided here (use the git.* tools). Package entries are per family: Python packages install through the `packages` argument or the python.packages tool, Node packages through npm/pnpm, signed WASI commands through the wasm.packages tool, and Debian packages through apt. Check unfamiliar commands with command -v. Use jobs.submit for long-running commands, or shell.open/shell.exchange for a persistent session. Output is capped; inspect exit status (124 timeout, 126 blocked, 127 not found, 75 not started — retry after the running command stops). The task environment selects the backend: the default Linux backend runs the command inside the environment’s TinyEMU Linux guest (Debian userland, real python3/node/apt, workspace mounted at /workspace); an environment explicitly set to the native compatibility backend keeps the iOS-hosted POSIX subset without python3/node. A stopped guest starts on demand; a missing Linux component fails with the install reason."
