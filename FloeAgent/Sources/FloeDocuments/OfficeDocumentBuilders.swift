...
public enum OfficeDocumentBuilder {
    ...
    // The presentation theme names "PingFang SC" — the system CJK family on
    // every supported device. The previous "Source Han Sans SC" is not
    // installed on iOS, so CJK glyphs in generated decks rendered as
    // empty squares (same root cause as the Word theme above).
