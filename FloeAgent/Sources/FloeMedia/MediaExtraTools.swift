...
public struct VideoRemuxTool: AgentTool {
    ...
    public static let toolDescription =
        "Copy streams into a different container without re-encoding (mp4, mov, m4v, caf, wav). Fails when the target container cannot carry the source codecs; no parameters are defaulted."
...
public struct VideoProxyTool: AgentTool {
    ...
    public func execute(_ args: Arguments, context: ToolContext) async throws -> ToolExecutionOutput {
        ...
        let result = try await engine.transcode(.init(
            ...
            videoCodec: nil,
            audioCodec: nil,
            width: args.maximumDimension,
            height: args.maximumDimension,
            frameRate: nil,
            videoBitrate: nil,
            audioBitrate: nil,
            passthrough: false
...
public struct AudioMixTool: AgentTool {
    ...
    public static let toolDescription =
        "Mix two audio files into a new output. Both paths and the output are required; optional per-input gains are applied exactly. Use audio.inspect first to choose levels."
