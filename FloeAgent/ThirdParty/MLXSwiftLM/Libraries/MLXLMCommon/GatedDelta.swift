...
public func gatedDeltaUpdate(
    ...
    // Floe local fix (Build227 crash hotfix): the fused Metal kernel runs the
    // whole T-step recurrence inside one async kernel dispatch. Multi-token
    // GDN prefill (T > 1, Qwen3.5/Qwen3-Next prompt processing) was suspected
    // of triggering an asynchronous Metal crash in that fused path, so prefill
    // is routed to the existing `gatedDeltaOps` recurrence (per-step MLX ops,
    // same numerics, no fused kernel). Single-token decode (T == 1) keeps the
    // fused fast path.
    //
    // The fused kernel also distributes Dk over exactly 32 simd lanes
    ...
    // Dk = 192 (a multiple of 32), but the value is config-driven, so even for
    // T == 1 any non-multiple-of-32 Dk uses the ops fallback, which handles an
    // arbitrary key dimension correctly (slower, but never truncating).
    if T == 1, GatedDeltaKernelManager.shared.kernel != nil, Dk % 32 == 0 {
