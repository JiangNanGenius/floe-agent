...
struct LocalPythonRuntimeTests {
    ...
    @Test("bundled XML and Office Python libraries save and reopen Chinese documents")
    @MainActor func bundledOfficeRoundTrips() async throws {
        let service = try #require(CPythonServiceFactory.make())
        let outcome = await service.run(.init(script: """
        import io
        import json
        ...
        """, timeout: 30, maxOutputBytes: 4096), cancellation: nil)
        guard case .ok(_, let stdout, let stderr, false, false, _) = outcome else {
            Issue.record("Bundled Office round trip failed: \(outcome)")
            return
        }
        #expect(stdout.contains(#""docx_pptx_round_trip": "passed""#))
        #expect(stderr.isEmpty)
    }
