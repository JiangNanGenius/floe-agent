...
/*
 * Read from so's socket into sb_snd, updating all relevant floe_slirp_sbuf fields
...
{
	struct floe_slirp_sbuf *sb = &so->so_rcv;
 ...
 * updating all floe_slirp_sbuf field as necessary
 */
int
...
sorecvfrom(struct socket *so)
{
	...
	if (so->so_type == IPPROTO_ICMP) {   /* This is a "ping" reply */
	  ...
	  if(len == -1 || len == 0) {
	    ...
	    DEBUG_MISC((dfd," udp floe_slirp_icmp rx errno = %d-%s\n",
	  ...
	  if(m->m_len<0) {
	    ...
	    DEBUG_MISC((dfd," rx error, tx floe_slirp_icmp ICMP_UNREACH:%i\n", code));
	    icmp_error(so->so_m, ICMP_UNREACH,code, 0,strerror(errno));
	    m_free(m);
	  } else {
	  /*
 ...
 */
int
sosendto(struct socket *so, struct floe_slirp_mbuf *m)
{
	Slirp *slirp = so->slirp;
	int ret;
	struct sockaddr_in addr;
