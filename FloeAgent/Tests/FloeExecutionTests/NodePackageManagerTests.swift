...
struct NodePackageManagerTests {
    ...
    func actualManagedNpmAndPnpmInstallSwitchRollbackAndRemove() async throws {
        ...
        for manager in NodePackageManager.allCases {
            _ = try await service.change(environment, specification: "is-number@7.0.0", remove: false, manager: manager, cancellation: CancellationToken())
            let outcome = await runtime.run(.init(entryScript: nil, arguments: ["-e", "console.log(require('is-number')(42))"],
        ...
        _ = try await service.change(environment, specification: "is-number", remove: true, manager: .pnpm, cancellation: CancellationToken())
