...
struct ArchiveBrowserServiceTests {
    ...
    @Test("bz2/tbz2 decode through the browser with the same bounds as gzip/xz")
    func bzip2DecodeSupported() async throws {
        let f = try Fixture()
        try f.write("note.txt", "bzip2 payload")
        try f.write("tree/a.txt", "alpha")
        try f.write("tree/b.txt", "beta")
        let service = f.service

        // Creation and decode are both native now.
        _ = try ArchiveEngine.create(
            format: "tbz2",
            sources: [f.root.appendingPathComponent("tree")],
        ...
        // The advertised capability matches the behavior: bz2/tbz2 are in the
        // native decode sets (no decode-unsupported carve-out remains).
        #expect(ArchiveBrowserService.nativeFormats.contains("tbz2"))
        #expect(ArchiveBrowserService.singleFileFormats.contains("bz2"))

        // Listing a tar.bz2 decodes it with the bounded streaming decoder.
        let containerListing = try await service.listing(relativePath: "pack.tar.bz2", rootURL: f.root, cancellation: f.cancel)
        #expect(containerListing.format == "tbz2")
        #expect(containerListing.entries.contains { $0.path.hasSuffix("tree/a.txt") })
        #expect(containerListing.entries.contains { $0.path.hasSuffix("tree/b.txt") })

        // Extraction writes the real tree.
        let summary = try await service.extract(
            relativePath: "pack.tar.bz2", destinationDir: "tbz2-out", rootURL: f.root, cancellation: f.cancel
        )
        #expect(summary.contains("entries=2"), "summary was: \(summary)")
        #expect(try f.read("tbz2-out/tree/a.txt") == "alpha")
        #expect(try f.read("tbz2-out/tree/b.txt") == "beta")

        // Single-file .bz2 decompresses to a new file.
        let bz2Summary = try await service.decompress(
            relativePath: "note.txt.bz2", destinationFile: "note-restored.txt", rootURL: f.root, cancellation: f.cancel
        )
        #expect(bz2Summary.contains("entries=1"))
        #expect(try f.read("note-restored.txt") == "bzip2 payload")
        ...
        // A corrupted bz2 is rejected with nothing committed.
        var corrupt = try Data(contentsOf: f.root.appendingPathComponent("note.txt.bz2"))
        corrupt[corrupt.startIndex + corrupt.count / 2] ^= 0xFF
        try corrupt.write(to: f.root.appendingPathComponent("corrupt.bz2"))
        do {
            _ = try await service.decompress(
                relativePath: "corrupt.bz2", destinationFile: "corrupt-out.txt", rootURL: f.root, cancellation: f.cancel
            )
            Issue.record("corrupted bz2 must be refused")
        } catch let error as ArchiveBrowseError {
        ...
        #expect(!FileManager.default.fileExists(atPath: f.root.appendingPathComponent("corrupt-out.txt").path))
    }
