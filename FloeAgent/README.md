...
[Build191](../docs/qualification/build191-release/README.md), immutable `715cbc42` / `v1.7.0-beta.48`, is in expedited internal distribution using recovery run35347141494 after the user explicitly waived all three recorded UI failures. Both SDK App suites passed204/204 and both NativeNotes components101/101. The two SDK27 UI failures and a newly completed accepted-SDK iPhone assistant-readiness failure remain recorded; all three are explicitly waived for internal device testing. Upload reuses the exact-source accepted-SDK artifact; signing and Apple/Floe QA validation remain required. No public Beta or production submission is authorized. Physical local-model and native Office/Pencil acceptance, full package/model delivery and RDP App integration remain open.
