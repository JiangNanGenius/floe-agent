...
This directory contains the Swift package, generated Xcode project, native app, tests, and release scripts for Floe 1.7. The minimum deployment target is iOS/iPadOS 26.0; database migrations now reach schema v40. Build 1.7.0 (178) remains the last confirmed internal Floe QA TestFlight release. The build 185 candidate (source `42ecc452`) passed both SDK App regressions 204/204 but failed Notes UI qualification, so upload was skipped. The current build 186 candidate (beta.43) is preparation only, with no tag or upload: its Office preview lifecycle fix is `2a907841`; component run `35301809882` passed iPad 84/84 and iPhone 83/84, where the only failure was a real scanned bilingual OCR/search case exceeding 120 seconds rather than Office. Current source `8d303197` applies the existing 180-second per-case ceiling to that single functional case with a pixel-identical fixture and unchanged text/search assertions, and component run `35304310882` is in progress; this is not a product performance claim. See [the beta.43 candidate notes](../docs/RELEASE_1.7.0_BETA_43.md) and [the Notes follow-up record](../docs/qualification/build185-release/notes-followup.json). RDP is not delivered as a usable App feature; native iPad local-model stability and native Office/Pencil remain physical-device acceptance. Full package/model delivery remains separate. See the [current build and acceptance guide](../docs/FLOE_1_7_BUILD_AND_ACCEPTANCE.md) before running builds.
