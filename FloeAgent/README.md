...
This directory contains the Swift package, generated Xcode project, native app, tests, and release scripts for Floe 1.7. The minimum deployment target is iOS/iPadOS 26.0; database migrations now reach schema v40. Build 1.7.0 (178) remains the last confirmed internal Floe QA TestFlight release. The build 185 candidate (source `42ecc452`) passed both SDK App regressions 204/204 but failed Notes UI qualification, so upload was skipped. The current build 186 candidate (beta.43) is frozen at source `d421fea260523d063270e2d21d623bd011acd9ea` and tagged `v1.7.0-beta.43`; [release run `35306551280`](https://github.com/JiangNanGenius/floe-agent/actions/runs/35306551280) started 2026-09-18 04:19:57 UTC and runs the NativeNotes development component plus both SDK App build/verify jobs in parallel, with upload gated on all three. At this record the run was in progress; nothing is uploaded or installable. Component run `35301809882` passed iPad 84/84 and iPhone 83/84, where the only failure was a real scanned bilingual OCR/search case exceeding 120 seconds rather than Office; follow-up run `35304310882` passed OCR on both devices with the effective XCTest timer reset from 120 to 180 seconds proven from the exported session logs, but still failed iPhone 83/84 on one direct staged Word Quick Look timeout while the same file passed through the real cover service (see `Local/Private/build186-final-review/REPORT.md`). The tagged source contains the six Office sample tests rewritten to the real importer/CAS/cover-service path with strict Quick Look assertions; the current release run is their first live execution. This is not a product performance claim. See [the beta.43 candidate notes](../docs/RELEASE_1.7.0_BETA_43.md) and [the Notes follow-up record](../docs/qualification/build185-release/notes-followup.json). RDP is not delivered as a usable App feature; native iPad local-model stability and native Office/Pencil remain physical-device acceptance. Full package/model delivery remains separate. See the [current build and acceptance guide](../docs/FLOE_1_7_BUILD_AND_ACCEPTANCE.md) before running builds.
